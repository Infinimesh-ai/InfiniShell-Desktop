...
def started(mode):
    return {"event": "selected_skill_started", "scope": SCOPE, "case": mode, "max_native_inputs": 1,
...
def validate_event(event):
    if (type(event) is not dict or type(event.get("case")) is not str
            or event["case"] not in {"leader", "sdk"} or event.get("scope") != SCOPE):
        return False
...
def boundary_passed(metadata, launches, tunnel, mode):
    expected = "private_leader" if mode == "leader" else "direct_agent"
...
def run(args):
    ...
    metadata = dict(observation(None, [], args.mode, expected_hash), scope=SCOPE, case=args.mode,
        private_workspace=str(root), test_name=TEST_NAME, max_native_inputs=1, expected_sha256=expected_hash,
    ...
    with isolation.bounded_tunnel(args.timeout) as tunnel:
        try:
            ...
            for key in ("default_entrypoint_verified", "product_selected_skill_combination_verified", "native_catalog_selected_path_seen_before_input"):
