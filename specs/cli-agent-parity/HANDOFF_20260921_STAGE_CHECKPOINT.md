...
- `84a174f9…` 已修复主线程恢复顺序并完成远端诊断：系统命令原生退出通过，非系统 DLL 拒绝未完成。现有日志没有把超时定位到事件等待还是映像校验，不得据此宣称根事件从未送达。
- 后续未提交候选已给严格 Job 增加恢复失败时移交子进程的专用入口，并确保当前创建进程事件没有可终止句柄时绝不 Continue；隔离本机 `cargo check -p warp`、Windows 目标 `command` check 和 i18n 11 项通过。`spawn_suspended()` 在线程发现失败时仍可能同步等待；所有新增错误路径都未进入 `84a174f9…` 的远端 run，须新 SHA Windows 实机复验，`ManualOnly` 不变。
...
- receipt115 的 run 35827672098 已完成结算；随后同一分支干净提交 `84a174f9…` 的 run 35837274454 也已结算为 Linux 通过、Windows 原子 DLL 拒绝诊断失败，详见 receipt120。下一步先给失败用例增加分阶段计时和有界期限，再用新 SHA 聚焦 Windows 实机验证；不要把历史运行中的状态继续当作当前状态，也不要只为刷新绿色结果重发。
...
7. receipt108／109 已完成 `c25221a22…` 的 Linux／Windows 聚焦预检及 Linux `execveat` 机制实测；receipt115 记录后续 Linux hook 和 Windows 严格 Job 失败，receipt120 已在 `84a174f9…` 上闭环 Linux hook 与系统命令原生退出，但非系统 DLL 拒绝测试继续超时。先定向诊断该用例并在新 SHA 上运行 Windows 严格 Job；之后按下述分层门禁节奏处理剩余范围：Codex 当前版完整产品生命周期与取消、Linux 三款产品原子升级事务、SSH/tmux 产品接收、GUI IME 与双语布局、异常矩阵，以及功能冻结后的同提交 macOS 与 full workspace 验证。
