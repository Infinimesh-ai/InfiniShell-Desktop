...
- Codex `0.155.1` 新增显式运行中工具取消验收夹具：要求真实原生工具开始、固定 Python 父子进程同时存活后才发取消，并核对原生 ACK、同代清理回执、父子零残留及取消终态顺序。该夹具和外层安全兜底通过离线测试与编译门禁后，仍须在当前源码的干净提交、同源签名 supervisor 和隔离认证副本上运行真实模型回合；源码或模拟事件不替代 receipt92 的失败闭环。
