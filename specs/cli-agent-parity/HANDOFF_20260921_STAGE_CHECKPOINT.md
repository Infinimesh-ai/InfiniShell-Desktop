...
- 本轮候选为 Codex `0.156.1` 增加独立 test-only Rust adapter 完整生命周期入口：固定完整包校验先于认证副本，每次新建／恢复会话核对精确版本；正式产品版本列表未改。运行器 Python 16／16、`cargo fmt --all -- --check`、独立内置 target 的 `cargo check -p warp` 及启用托管 feature 的 libtest 编译通过；真实模型回合尚未执行，不能据编译结果开放正式门禁。此改动无用户可见文案变化，无需本地化变更。
...
- 2026-09-23 14:42 UTC 再次检查时，ACASIS 与 ORICO 均不在 macOS 磁盘或 USB 设备列表，`/Volumes/ACASIS` 不存在；这次不是可点击的权限弹窗，且无法对不存在的设备执行挂载。内置盘当时约 93 GiB 可用；Python 验证显式指定 `/private/tmp` 后通过 16／16。临时将仓库 `target` 链接指向独立内置临时 target 后，`cargo check -p warp` 与启用 `local_cli_managed_tasks` 的 `cargo test -p warp --lib --no-run` 均通过；每次命令退出后已恢复原 `target` 链接。外置盘原缓存未清理或迁移，签名 supervisor 构建与新版 Codex 在线实链仍待验。
