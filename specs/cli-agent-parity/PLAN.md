...
> **receipt102–104 阶段收敛**：Claude 官方订阅账户与固定 `2.1.278` 已改为隔离设置源、原生 manual/default 审批合同；定向测试通过，但最终父子实链在外置调试 supervisor 的 macOS launchd 接管阶段以 EAGAIN 中断，未产生 SessionReady、父子 ACK、进度、结果或恢复。Grok `1.0.40` 已补精确认证、setup 与目录形状门禁，但真实 fresh-home 多次运行出现握手通知跨运行竞态，最终仍在模型输入前 fail closed，生产 submit／审批／取消／恢复及扩展能力均未开放。Windows 三款官方 x64 CLI 的版本和 Authenticode 已实机核对；cwd 可重命名、调试树不退出、强杀后根进程残留及 Codex 官方资产漂移证明当前原子更新必须继续 `ManualOnly`。边界分别见 [receipt102](validation/macos-working-tree-102-claude-authorized-parent-child-current.safe.json)、[receipt103](validation/macos-working-tree-103-grok-1040-root-lifecycle-current.safe.json) 与 [receipt104](validation/windows-working-tree-104-atomic-real-cli-failclosed.safe.json)。本阶段没有完成 SSH／tmux、GUI IME／双语布局、Linux 实际执行或同提交跨平台终验；Goal 不得标记 complete。

> **receipt100 当前进度**：在 receipt99 的原子执行实现上，macOS 私有快照现会逐字节镜像源扩展属性，修复 `fcopyfile` 重写 `com.apple.quarantine` 时间字段导致官方签名 CLI 摘要不一致的问题；新增隔离回归后 macOS atomic 15／15、系统临时卷上的 launchd 实际 C 夹具 1／1 通过。当前同源产品用例已真实完成 Claude `2.1.267`→`2.1.278` 和 Grok `1.0.34`→`1.0.40`，两者均为 inspect 2 次、execute 1 次、模型输入 0、配置字节与旧／目标参考不变、journal 清理且生产链绑定通过。此前不安全祖先拒绝、扩展属性失败和 Grok 网络失败收据原样保留。精确边界见 [receipt100](validation/macos-working-tree-100-auth-autoupdate-live.safe.json) 与 [receipt99](validation/macos-working-tree-99-atomic-update-current.safe.json)。
>
> 当前父子恢复已补终态结果幂等修复及普通消息的持久入队→原子 `Sent` claim→真实 `MessageAccepted` ACK。Grok OAuth、Claude 兼容 Anthropic API 与 Claude 官方在线账户均已获用户授权；独立 CLI 结果按 receipt100 保留。[receipt101](validation/macos-working-tree-101-claude-authorized-account-live.safe.json) 又以官方 Claude 2.1.278 和生产 adapter 实际完成两轮、允许／拒绝、运行中后续输入、取消后继续及新进程同原生会话恢复，隔离 OAuth 的真实失败也原样保留。该收据仍不是应用 GUI 重启、父子任务、冻结提交或跨平台证明。Grok 1.0.40 追加、技能、本地工具、子任务、父子双向 ACK／进度／回收／恢复仍待当前版本真实闭环。Linux／Windows 实际执行、三款最终完整生命周期、SSH／tmux 产品接收、真实 IME／双语布局、冻结提交与推送均未完成；Goal 保持 active。
...
> **同日后续收敛更正**：Codex 候选现已把未完成原生执行对应的取消／完成／失败终态全部延迟到同代托管清理回执和有界尾部读取之后；非取消结果会降为失败，任一清理或投递错误都不发布终态。Runtime Host 在进程创建前持久化 `startup-incomplete`，spawn 失败先写 `not_started` 再清标记；启动 future 被取消时只请求终止并保留 `NativeExitUnconfirmed`，协调器不会自动重投，但尚无独立监视收据，故仍不算恢复完成。自动升级 journal 现保存实际 manager program、参数与 cwd，旧或畸形 pre-spawn journal 不再生成伪收据；监督 manifest 到 OS spawn 的 witness 空窗及 pathname 执行仍未关闭。三部分仅完成源码与静态收敛，当前依据为 receipt95；receipt93 只保留修正前历史，Cargo／真实产品／跨平台仍未验。
