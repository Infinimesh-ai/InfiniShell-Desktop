...
本文件说明当前分支的产品支持范围与回退行为，不表示已经发布。固定版本的功能证据见 [验收对应表](https://github.com/Infinimesh-ai/InfiniShell-Desktop/blob/e3ef39689cd8b686dfe040b90217ed1067b4d268/specs/cli-agent-parity/ACCEPTANCE_FIXED_VERSIONS_20260924.md) 和 [Mac 交付记录](https://github.com/Infinimesh-ai/InfiniShell-Desktop/blob/e3ef39689cd8b686dfe040b90217ed1067b4d268/specs/cli-agent-parity/MAC_FIXED_VERSION_DELIVERY_20260924.md)；历史对应提交的三平台必要补验已通过；本次输入增量（提交绑定待补）尚未取得最终 SHA 门禁，实际范围与来源以 [验证结论](VALIDATION_REPORT.md) 为准。不同提交的历史通过不能改记为最终提交通过。
...
- 本次新增 PNG 托管图片仅在已核验 `1.0.41/grok-4.7`、Inherit、无已选技能根会话开放；未知版本/模型、固定策略及技能混用仍拒绝。macOS 生产适配器已有新建/冷恢复正例，完整产品与跨平台验收未完成。目录、技能、权限或会话身份未通过核验时，相关操作保持不可用，不能从安装完成或菜单出现推断能力就绪。

## 输入与技能的实际限制

- 普通 Grok 终端的富输入发送目前会拒绝自动提交，提示复制草稿后到原生输入框粘贴；图片粘贴也未开放。托管 Grok 文本输入可用，PNG 图片按上述限定范围接通（G01–G03）；普通终端 Ctrl+V 原生正例不意味着产品自动发送已开放。
- Claude `2.1.280` 托管输入已扩展 PNG、JPEG、静态 GIF、WebP，允许纯图片及精确登记的单技能组合。格式及纯 PNG 已有 macOS/claude-opus-5-5 生产适配器字节、识图和冷恢复正例；图片加技能仍待完整生产链验收。GIF 单帧校验也约束旧引用，不改变 Codex 原格式行为（G04／G05）。
...
- Codex GUI 已证明类型化图片进入原生请求，该轮模型颜色判断错误不能计作理解正确；Claude 历史 GUI PNG 有字节和识色证据；本次其他静态格式/纯 PNG 及 Grok PNG 的生产适配器正例另列，不冒充新格式 GUI 或其他平台验收。
...
本次输入增量已同步英文与简体中文图片/文件说明；macOS 重启后两种语言的 Claude 格式说明及 Grok 顶部说明均可读。使用 WIP 二进制与动态资源，只有布局证据，不能认作最终 SHA 产品或在线链验收；最终构建及门禁仍待补。详见 [当前增量验证](VALIDATION_REPORT.md#2026-09-25-继续实现增量)。
