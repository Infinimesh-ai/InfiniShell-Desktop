...
| --- | --- | --- | --- |
| P0 身份、协议和能力；R1 Grok 扩展 | `app/src/ai/cli_agent_runtime/{codex,claude,grok}.rs`、`grok_profile.rs` | 固定三款，Mac 正式托管；Grok 固定读取、文件、单技能和原生目录均有实链；[目录刷新 V11](validation/macos-fixed-versions-20260924/gui-grok-idle-catalog-v11.safe.json)覆盖最终目录字段修复 | Mac 对应能力通过；Linux／Windows 原生边界与确定性回归已有平台证据，剩余项见 P5 |
...
| P5 平台、提交和报告；R8 | `.github/workflows/cross-platform-preflight.yml`、本表和验证收据 | [源码对应](validation/macos-fixed-versions-20260924/source-binding-981cfebc4.safe.json)、[Mac 复用审计](validation/macos-fixed-versions-20260924/ci-981cfebc4-mac-source-reuse.safe.json)；Linux／Windows 全量 10,899／10,660 项通过按原提交保留；Linux 与 Windows 真实 GUI 已通过；`72caebfc8` 的[Windows 两项补验](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36026855842)执行 Grok 插件迁移与 Codex 原子更新 | 待实际结果与报告闭环；未重跑已通过且源码未变的全量范围，旧 GUI 二进制不重标 |
