...
本表把原 PLAN P0–P5、REOPEN R1–R8 与本轮实际证据对应。版本固定为 Codex `0.156.1`、Claude Code `2.1.280`、Grok Build `1.0.41`，不因上游发布新版本重新开始本轮。本表记录已验子集，不能覆盖[统一缺项](KNOWN_GAPS.md)或证明完整 Goal 完成；当时受测产品提交为 `b3acf2a69010f9bd776558ce4f2fae9fbaa3e511`。[最终收据](validation/macos-fixed-versions-20260924/goal-fixed-versions-final.safe.json)绑定实际源码、版本与平台，后文历史失败保留。
...
| --- | --- | --- | --- |
| P0 身份、协议和能力；R1 Grok 扩展 | `app/src/ai/cli_agent_runtime/{codex,claude,grok}.rs`、`grok_profile.rs` | 固定三款，Mac 正式托管；Grok 固定读取、文件、单技能和原生目录均有实链；[目录刷新 V11](validation/macos-fixed-versions-20260924/gui-grok-idle-catalog-v11.safe.json)覆盖最终目录字段修复 | 所列模式通过；P1、P3／P4 模式限制与 P5 覆盖不足另见 G01–G13、V01–V05 |
| P1 富输入、附件、上下文、技能、评审；R6 | `task_manager_input.rs`、`task_manager_view.rs`、终端 `use_agent_footer` | [Codex GUI](validation/macos-fixed-versions-20260924/gui-codex-composer-v2.safe.json)、[Claude 技能／图片／恢复](validation/macos-fixed-versions-20260924/gui-claude-composer-recovery-v7.safe.json)、[Grok 技能](validation/macos-fixed-versions-20260924/grok-selected-skill-v7.safe.json)、[真实 Mac IME](validation/macos-fixed-versions-20260924/ime.safe.json) | 所列输入路径通过；G01–G08 仍缺实现或受限，V02／V04／V05 未完整验收，不计 P1 全部完成 |
...
| P3 权限和配置保全；R5 默认入口 | `permissions.rs`、`grok_profile.rs`、`app/src/features.rs` | 固定三款明确审批、父权限上限、默认构建 GUI；[V11 默认构建](validation/macos-fixed-versions-20260924/build-default-goal-v11.safe.json)没有托管编译 feature | 正式入口和相应策略通过；Claude／Grok 固定策略与继承模式限制见 G10 |
| P4 父子任务和双向 ACK；R2 | `coordinator.rs`、`runtime_host.rs`、持久任务和信箱 | [Codex V9 双向 ACK](validation/macos-fixed-versions-20260924/codex-parent-child-duplex-v9.safe.json)、[Claude 父子](validation/macos-fixed-versions-20260924/native-summary.safe.json)、[Grok V8 父子冷恢复](validation/macos-fixed-versions-20260924/grok-parent-child-v8.safe.json) | 三款限定模式的原生链通过；子任务能力受 G10 限制，显式继续、ACK、持久 inspect 分别计证 |
...
| P5 平台、提交和报告；R8 | `.github/workflows/cross-platform-preflight.yml`、本表和验证收据 | [最终源码对应](validation/macos-fixed-versions-20260924/source-binding-b3acf2a69.safe.json)、[未变域复用审计](validation/macos-fixed-versions-20260924/ci-72caebfc8-source-reuse.safe.json)；Linux／Windows 全量 10,899／10,660 项通过按原提交保留；Linux 与 Windows 真实 GUI 已通过；`b3acf2a69` 的[Windows 两项补验](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36028161259)已通过 Grok 插件迁移与 Codex 原子更新 | 该轮相关检查通过，V01–V05 仍有覆盖不足；Windows 571 项回归、严格 Job 和清理按原范围计证，旧 GUI 不重标 |
...
修正已推送为 `981cfebc4e3c9acfde48140bddb5d67134c282ab`，与远端主工作分支和独立验证引用一致。[源码对应](validation/macos-fixed-versions-20260924/source-binding-981cfebc4.safe.json)绑定本批 35 个变更输入；[集中补验 36015553498](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36015553498)已派发 Linux GUI 专项及 Windows 原生／GUI，完整工作区与 Mac Intel 关闭。当前仍待真实结果。

`36015553498` 的 [Linux 原始失败](validation/macos-fixed-versions-20260924/ci-36015553498-linux-apt-index-failure.safe.json)发生于 CJK 字库安装前的 Ubuntu APT 索引同步：下载索引大小／摘要与 Release 不符，严格校验退出 100。应用 check 和 GUI 编译已通过，但字体覆盖、真实 GUI 和截图均未运行；缺少 artifact 是继发失败，不能计产品 GUI 失败或通过。Windows 作业继续执行。

APT 依赖获取已按[静态收据](validation/macos-fixed-versions-20260924/ci-linux-apt-fetch-fix.safe.json)修正并推送为 `9009af078456782031e135cf321ef21a53f31808`：先用已有认证索引，必要时强制 By-Hash 与有界严格刷新，不关闭签名或摘要验证。[源码对应](validation/macos-fixed-versions-20260924/source-binding-9009af078.safe.json)确认产品／测试脚本与 `981cfebc4` 一致；[Linux GUI 单项补验 36018303373](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36018303373)已派发，Windows 原作业保持。

Windows 固定 Codex `0.156.1` 的 [hook 与 ConPTY 收据](validation/macos-fixed-versions-20260924/ci-36015553498-windows-codex-hooks-conpty.safe.json)通过，两种路径各自两个场景均根自然退出 0、配置恢复且零模型请求；特殊字符场景中独占 Job 在宽限后回收 5 个后代，活动数归零并取得双流真实 EOF。后代强制清理与根自然退出分别记录；ConPTY 仅证明 session_start／prompt_submit，外层驱动 Job 未证明、普通 TUI 与 GUI 通知未由此证明。

Linux `36018303373` 的[原始 GUI 结果与目视失败](validation/macos-fixed-versions-20260924/ci-36018303373-linux-gui-font-failure.safe.json)保留：CJK 字库与逐字覆盖、应用 check、GUI 编译、中文内容与图片像素断言均通过，但实际两张 PNG 的中文仍为方框，且摘要与上一轮失败相同。作业绿灯不计 GUI 验收通过，正在排查实际字体选择路径。

Windows Grok [同次 cmd 语法错误](validation/macos-fixed-versions-20260924/ci-36015553498-windows-grok-cmd-syntax.safe.json)已明确发生于进入 PowerShell 前；[路径修正](validation/macos-fixed-versions-20260924/ci-windows-grok-cmd-path-fix.safe.json)仅将测试夹具的 `System32/cmd.exe` 改为逐组件连接，生产模板和参数不变。[安装器专项模式](validation/macos-fixed-versions-20260924/ci-windows-grok-installer-only-workflow.safe.json)保留 cargo check、相关回归、原生六阶段安装及失败回执，跳过源码未变且已验证的其他能力。当前本机 ACASIS 未挂载，生产源码与已通过 cargo check 的 `981cfebc4` 完全相同；不声称重新执行了本机 Cargo，修复后 Windows 原生结果仍待验。无需本地化变更。

安装器测试修正已提交并推送为 `4880b6cf2d46ee9dd1b552e475a14405375b8d37`，[源码对应](validation/macos-fixed-versions-20260924/source-binding-4880b6cf2.safe.json)明确生产行为与 `981cfebc4` 相同；[Windows Grok 单项补验 36021674501](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36021674501)已排队。原 Windows 更新／GUI 作业继续，不被新补验取消。

Windows [本轮正式原子更新](validation/macos-fixed-versions-20260924/ci-36015553498-windows-atomic.safe.json)确认 Claude 与 Grok 通过：实际退出 0、入口目标摘要／版本、更新后来源复核、13 项配置保全、严格 Job 清理与 journal 清除均通过。Codex 已不再拒绝系统 DLL，但正式更新及独立诊断均原生退出 1，旧入口与配置保留；仍计失败并继续定位，不把无拒绝映像等同于更新成功。

Windows [真实 GUI 收据与原始截图目视](validation/macos-fixed-versions-20260924/ci-36015553498-windows-gui.safe.json)已通过：`981cfebc4` 构建的 `a63ae170…` integration EXE 经固定 Mesa GL/llvmpipe 创建真实窗口，中文多行可读、图片附件可见、文本及像素一致、零任务／零模型输入，两张 PNG 摘要绑定同一收据。不把该系统剪贴板验收扩称为物理 IME 或 DXGI 全功能通过。[本轮 Windows 作业](validation/macos-fixed-versions-20260924/ci-36015553498-windows-final.safe.json)的原始失败仍仅归属 Grok 安装器和 Codex 更新。

Linux Fontconfig 修复已提交并推送为 `175b8de4d1b90914cad37a55766add1cd1a07058`：[源码对应](validation/macos-fixed-versions-20260924/source-binding-175b8de4d.safe.json)、[修复与缺字门禁](validation/macos-fixed-versions-20260924/ci-linux-cjk-fallback-fix.safe.json)、[本机 check 与环境恢复](validation/macos-fixed-versions-20260924/ci-linux-cjk-fix-local-gates.safe.json)分别留证。[Linux GUI 单项 36023402924](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36023402924)只执行相关 check、i18n、GUI 编译和真实截图，不重跑 Linux 原子事务或全量工作区。

Windows Grok 单项 [36021674501 的原始失败](validation/macos-fixed-versions-20260924/ci-36021674501-windows-grok-migration-failure.safe.json)已跨过 cmd／PowerShell 特殊参数、生产新装、原生 inspect 和 0.1.4 hook 导出；0.1.3→0.1.4 生产 update 失败。旧收据中的命令退出 0 属于先前桥接，不能用于迁移判断，根因尚未知。Codex 的[原生合同补丁](validation/macos-fixed-versions-20260924/ci-windows-codex-native-contract-fix.safe.json)补足固定 OS 夹具、旧映像同一性、官方目标摘要与受保护编译器依赖；[本机 check、i18n 11 项及目标摘要回归](validation/macos-fixed-versions-20260924/ci-windows-codex-contract-local-gates.safe.json)通过，仍待 Windows 实链。无需本地化变更。

Linux [36023402924 真实 GUI](validation/macos-fixed-versions-20260924/ci-36023402924-linux-gui.safe.json)已通过：`175b8de4d` 的实际字体排版六汉字均无缺字，系统剪贴板文本／图片像素、零任务和零模型请求断言通过；两张原始 PNG 已逐张目视确认中文两行清晰、图片附件可见。没有物理 IME 或 FreeBSD 扩展声明，旧方框截图继续保留。

Grok 旧桥接夹具发现 [Windows 原始／规范化路径的合成字节差异](validation/macos-fixed-versions-20260924/ci-windows-grok-bridge-root-synthetic.safe.json)；[夹具修正及当次安全诊断](validation/macos-fixed-versions-20260924/ci-windows-grok-migration-fixture-fix.safe.json)复用生产 `grok_home_dir()` 原始根拼写，保留隔离断言和精确捕获校验，未改生产策略。静态证明不重写原失败原因，下一次原生迁移结果仍待验。

本批修正已冻结并推送为 `72caebfc80cd06a7c38e77ae47b29e8ed1f15075`，[源码对应](validation/macos-fixed-versions-20260924/source-binding-72caebfc8.safe.json)、[未变域复用](validation/macos-fixed-versions-20260924/ci-72caebfc8-source-reuse.safe.json)、[本机 check／Grok 62 项／i18n 11 项](validation/macos-fixed-versions-20260924/ci-windows-combined-final-local-gates.safe.json)均已留证。[Windows 专项 36026855842](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36026855842)只补 Grok 安装迁移和 Codex 正式原子更新及直接相关回归；Linux／Mac／全量／GUI 均不重复派发。

Windows [36026855842 编译原失败](validation/macos-fixed-versions-20260924/ci-36026855842-windows-exhaustive-match-failure.safe.json)为 `sources.rs` 新增 Windows match 未覆盖公共 `CLIAgent` 的另外 17 项，错误 E0004；原生 Grok／Codex 用例均未开始。修正显式覆盖全部 20 项，其他 CLI 返回 UnsupportedSource，不改变目标两款的原生更新合同。Mac check 不编译该 Windows 分支，不能代替 Windows 编译结果。

穷尽分支修正已推送为 `b3acf2a69010f9bd776558ce4f2fae9fbaa3e511`，[源码对应](validation/macos-fixed-versions-20260924/source-binding-b3acf2a69.safe.json)与[本机 check／i18n](validation/macos-fixed-versions-20260924/ci-windows-exhaustive-match-local-gates.safe.json)留证。[Windows 专项 36028161259](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/36028161259)沿用同一最小验证范围，前次原生用例未执行，不记作通过或原生失败。

Windows [Grok 六阶段原生安装／迁移](validation/macos-fixed-versions-20260924/ci-36028161259-windows-grok.safe.json)已在 `b3acf2a69` 通过：实际 0.1.3→0.1.4 生产 update、已装模块版本导出、同版本修复、禁用保全、文件事务故障回滚及后续生产修复均完成，cmd／PowerShell 参数保持，零模型请求且私有根已清理。模块导出不扩称为此次普通 TTY main／CONOUT／GUI 通知；历史失败原样保留。

Windows [最终专项](validation/macos-fixed-versions-20260924/ci-36028161259-windows-final.safe.json)全部通过：相关 Rust 571 项、进程所有权 2 项、严格 Job 调试 2 项和监督清理 4 项；[Grok 六阶段安装／迁移](validation/macos-fixed-versions-20260924/ci-36028161259-windows-grok.safe.json)及 [Codex 0.155.1→0.156.1 正式升级](validation/macos-fixed-versions-20260924/ci-36028161259-windows-codex-atomic.safe.json)均有真实收据。Codex 两次 inspect／一次 execute、目标入口版本／摘要、13 项配置、严格 Job 与 journal 清除通过；至此该轮选定专项通过；原“所有必需项完成”判断撤回，当前未完成项见[统一清单](KNOWN_GAPS.md)。最后改动未新增用户文案，无需本地化变更；本机 check、i18n 11 项及 Windows check 均通过。
