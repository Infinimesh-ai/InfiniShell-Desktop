...
原生 Grok 插件，版本 `0.1.2`，受测 Grok `1.0.30`。无需远端 marketplace。插件仅把已知生命周期事件转换为 InfiniShell OSC 777 v1 通知，不改变权限策略。
...
安装、更新或启用后，在运行中的 Grok 输入 `/plugins`，在 **Plugins** 页按 `r` 重新加载。真实 1.0.30 GUI 升级验证发现，仅在 Hooks 页按 `r` 或退出再打开前台会话可能保留旧插件缓存；需要先刷新 Plugins 页的注册表，再检查 Hooks 页。重新加载后应能看到本插件的当前版本及九项 hooks，应用收到当前通知后才能确认通知链路可用。
...
- JSON 文本转义终端控制字符；tmux 使用 DCS passthrough。Unix 优先写 `/dev/tty`。Grok 1.0.30 的 hook 会脱离控制终端，此时通过当前 tmux pane 的 PTY，或由 InfiniShell shell 引导刷新后的 `WARP_CLI_AGENT_TTY` 发送；未引导的 SSH shell 可使用 `SSH_TTY`。tmux 解析失败时不回退外层路径。Windows 独立写 `CONOUT$`。只写真实终端，无可靠目标时安静降级，不污染 ACP/hook stdout。
