...
- [receipt145](validation/macos-working-tree-145-a25701d2-claude-21280-parent-child-owned.safe.json)：Claude `2.1.280` 在当前 dirty 源码的 test-only 父子实链通过父权限上限、子任务文件效果、双向原生 ACK、自动结果交付及两代清理。receipt142 的首次失败保留；正式新版门禁及最终同 SHA 全范围验收仍关闭。
- [receipt146](validation/macos-working-tree-146-a25701d2-grok-1041-p0-external-failures.safe.json) 和 [receipt147](validation/macos-working-tree-147-a25701d2-grok-1041-p0-internal-missing-catalog.safe.json)：Grok `1.0.41` P0 两次外置盘尝试和一次内置盘复测均未完成第二会话。内置盘复测排除外置 Unix socket 长路径这个单独原因：第一会话 29 条目录、零输入握手和清理通过；第二会话收到原生 `session/new`、设置与公告，但 ACP 未交付命令目录，初始化超时，0 次业务模型输入。leader 内部广播不是 ACP 交付回执；不开放 P0 或正式版本门禁。私有认证副本、监督状态和隧道已清理。
- 用户最新允许本阶段使用内置存储；本轮独立 libtest 及签名 supervisor 的摘要固定，`/private/tmp` 用于临时状态。ACASIS 历史备份和私有失败证据保留，仓库 `target` 链接未改。以上无用户可见文案变化，无需本地化变更。

## receipt115–128 后续增量（2026-09-23）

- [receipt132](validation/version-discovery-132-20260923-latest-drift.safe.json)：2026-09-23 13:37 UTC 的官方来源核对显示 Codex 稳定版已为 `0.156.1`、Claude latest 为 `2.1.280`，隔离 Grok stable 查询仍为 `1.0.41`。官方 Codex macOS arm64 完整包摘要与签名、内置盘 `--version`、无凭据零输入 app-server initialize／只读 thread/start／EOF 通过；Claude 官方 manifest 与二进制摘要、严格签名及内置盘 `--version`／`--help` 通过。两款新版仅新增显式可选的固定包准备支持，现有默认版本与正式运行时门禁不变；Codex 准备器 51／51、Claude 准备器 12／12、`cargo check -p warp` 通过。预检没有任何模型输入，不是新版产品任务、审批、取消或自动升级验收。旧版实链继续按原 SHA 保留，最终目标需更新；无用户可见文案变化，无需本地化变更。
- [receipt134](validation/macos-clean-commit-134-952c0ec0-claude-21280-no-credentials.safe.json)：干净 `952c0ec0…` 的 macOS arm64 以内置盘固定官方 Claude `2.1.280` 通过无凭据原生 initialize／空闲 stdin EOF；仅 1 次控制请求与响应、0 模型输入、0 待审批、正常退出 0、未强制清理，私有记录审计后删除。正式产品适配器、在线任务、审批、取消、升级及同 SHA 其他平台尚未验收。新版本的显式 test-only 适配器候选已通过离线 Python 和 Rust 精确版本门禁测试，当前正式版本支持列表不变；无需本地化变更。
- [receipt135](validation/macos-clean-commit-135-2c00b045-codex-01561-missing-session.safe.json)：干净 `2c00b045…` 上，Codex `0.156.1` 的固定完整私有包与真实 Rust test-only adapter 通过无凭据缺失会话错误路径；0 模型输入、原生错误类别精确匹配、同代 macOS coalition 清理确认、staged 进程零残留。正式运行时新版门禁仍关闭，未验证有模型任务或升级。
- [receipt136](validation/macos-working-tree-136-2c00b045-claude-21280-lifecycle.safe.json)：同一 `2c00b045…` 源码的 Claude `2.1.280` 在线 Rust test-only adapter 基本链通过：已授权默认账户、文本与精确写入允许／拒绝、排队输入原生确认、取消、进程重启后继续及两代清理回执。运行时因未跟踪 receipt135 使工作树为 dirty，只记阶段性候选成功；GUI／App 重启、父权限上限、SSH／tmux、跨平台、三款原子升级和最终同提交验收仍缺失，正式新版门禁未开放。
- [receipt133](validation/cross-platform-preflight-133-e15224ad.safe.json)：旧源码 `e15224ad…` 的 Windows 专项 run 35867251565 已成功结算，此前 Codex Python 夹具失败项本次为 `ok`，生命周期 2619／2619、宿主崩溃监督 4／4；Linux、严格 Job 原子调试、GUI integration 及全工作区均跳过。5 份 artifact／24 文件通过格式及常见敏感模式审计；生产通知安装器并不证明原生 hook、App 或真实 CLI 升级。Jobs API 两个尾部步骤结论未收敛，原始日志与 Job success 的边界已保留，不能把旧提交结果计入最终同 SHA 验收。
- [receipt137](validation/macos-clean-commit-137-a25701d2-codex-01561-lifecycle.safe.json)：干净 `a25701d2…` 上，固定官方 Codex `0.156.1` 和同源签名 libtest／release supervisor 的 test-only Rust adapter 完成 7 个回合：双轮文本、固定命令允许与拒绝及文件效果、追加指令、取消、进程重启后恢复。无模型 supervisor 预检 2／2，真实在线链 49 条安全事件；App／GUI、运行中工具树取消、SSH／tmux、跨平台和升级仍未由此通过，正式版本门禁不变。
- [receipt139](validation/macos-clean-commit-139-a25701d2-claude-21280-lifecycle.safe.json)：同一干净 `a25701d2…` 上，Claude `2.1.280` test-only Rust adapter 以已授权默认账户重做在线基本链，56 条安全事件、8 个回合终态，允许／拒绝、排队输入、取消及重启后继续通过。此结果修复 receipt136 的 dirty tree 证据边界；HTTP 模型请求次数、App／GUI、父权限上限、SSH／tmux、跨平台和升级仍未验收，正式门禁仍关闭。
- [receipt140](validation/macos-clean-commit-140-a25701d2-grok-1041-selected-skill.safe.json)：同一干净 `a25701d2…` 上，Grok `1.0.41` test-only 默认 leader 先通过零输入目录，再以一次真实输入完成所选本地技能、精确只读审批、上下文读取及最终历史匹配。认证副本与网络隧道按边界清理；正式版本门禁、GUI、SSH／tmux、跨平台和升级仍未验收。
- [receipt141](validation/macos-working-tree-141-a25701d2-codex-01561-running-tool-cancel.safe.json)：在 `a25701d2…` 的 dirty 增量上，新 test-only Codex `0.156.1` 运行中工具取消通过原生工具开始、审批、取消 ACK、同代资源域清理、父子进程零残留和终态五事件顺序；外层审计无兜底清理。Rust check 与 libtest 编译通过；仍需干净提交复验，不能代替正式版本开放、GUI／SSH 或升级。
- [receipt142](validation/macos-working-tree-142-a25701d2-claude-21280-parent-child-failclosed.safe.json)：Claude `2.1.280` 专用测试构建的父子实链首次失败。签名、标记和无模型 supervisor 2／2 通过；真实父任务请求且获准 `run_agents`，保存的固定权限证明因父版本仍按正式列表校验而在创建子任务前被拒。父任务结束、子任务 0、文件未变、监督退出；测试候选修正及重测尚未完成，正式门禁未开放。
- [receipt138](validation/cross-platform-preflight-138-a25701d2.safe.json)：`a25701d2…` 的 Linux／Windows 聚焦 [run 35878568534](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/35878568534) 结论为 failure：Linux 34 成功／2 跳过；Windows 28 成功／2 失败／18 跳过。首败是 Codex `0.156.1` 完整包测试夹具 3 项在入口路径 guard 提前抛错；后续原生边界步骤跳过，上传缺文件为第二失败。3 个 artifact／17 文件在仓库外审计，GUI、full workspace、严格 Windows 原子调试及真实升级未验证；本机夹具路径修正仍须新 SHA 的 Windows 原生复验。
...
- [receipt130](validation/macos-clean-commit-130-a7c7a688-grok-1041-turn-failed.safe.json)：`a7c7a688…` 的 Grok `1.0.41` 真实 Rust adapter 新输入虽获原生 ACK，但 test-only 候选在审批版本匹配处拒绝，原生随后取消；失败收据保留。[receipt131](validation/macos-clean-commit-131-09798856-grok-1041-selected-skill.safe.json)：仅放开 test-only 已绑定技能的精确 read_file 审批后，干净 `09798856…` 的 macOS arm64 固定官方版本、同源签名内置盘测试程序与 supervisor，以新的唯一输入完成默认 leader 目录、原生 ACK、只读审批、上下文读取、3 条工具事件、最终历史与随机标记匹配，runner 退出 0、认证副本／隧道／私有根清理通过。runner 本身未核对 Git SHA，HTTP 模型调用数不可观察；正式 `1.0.41` 产品版本门禁、GUI、SSH／tmux、跨平台同提交及完整 Goal 仍关闭。用户可见文案未变，无需本地化变更。
- [receipt129](validation/cross-platform-preflight-129-cddafab7.safe.json)：[run 35856999722](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/35856999722) 精确绑定 `cddafab7…`，Linux job 成功，Windows job 失败。Windows 首个 Python 失败是夹具在 `taskkill` 平台分支仍断言 `os.kill` mock；后续原生边界生产者跳过导致 artifact 上传缺文件。严格 Job 两项原生机制测试首试通过，但 Windows 原生恢复、安装器等跳过项未验；3 份 artifact／17 文件结构、JSON／NDJSON 与常见敏感模式审计通过。后续源码修正不能回填该 run 的结论。
...
- [receipt111](validation/macos-working-tree-111-grok-1040-selected-skill-combination.safe.json)：新的预验证观察点证实官方 `1.0.40` 默认 leader 实际交付 30 条目录（含第 9 位唯一选定技能），旧 29 条静态验证器是首个可修复适配缺口；receipt110 的当时未观察到目录不能再被外推为 CLI 从不交付。仅对单技能的名称、规范路径与目录形状作精确增量后，macOS 隔离工作树的零输入目录握手退出 0；随后 1 次真实输入／接收、1 次精确只读审批、1 次上下文读取、3 次原生工具事件、唯一 slash 最终历史与随机标记匹配，默认入口组合退出 0，原生清理和认证副本移除确认。直连 `--no-leader` 仅送达不同的 10 条目录并被拒绝；不算产品直连支持。receipt111 尚非干净提交、GUI、同提交跨平台或完整 Goal 通过，正式门禁仍关闭。
- [receipt112](validation/cross-platform-preflight-112-2c4880f-windows-debug-fixture.safe.json)：run 35815324806 精确绑定 `2c4880f…`，Linux x64 34 步成功、Windows x64 44 步成功但严格 Job atomic image-debug 诊断失败。日志证实两个 ignored 测试的内层 `--exact` 均运行 0 项，随后外层在读取不存在的 `native-exit.txt` 时失败；这是测试名带多余 crate 前缀的夹具缺陷，未实际验证原子 debug 机制。6 个 artifact／37 个文件已下载并做结构、JSON／NDJSON 与常见敏感模式核验。当前源码只对夹具名做最小修正，仍须新提交 Windows 实机重跑，原子产品升级及总 Goal 均不能算通过。
- [receipt113](validation/macos-clean-commit-113-grok-1040-selected-skill.safe.json)：用户重新登录后本机默认 `grok` 指向 `1.0.41`；验收继续使用摘要固定的 `1.0.40`，不混合版本。干净提交 `4b12091e…` 的 feature libtest 和签名 release supervisor 构建通过；旧 supervisor 从外置盘启动时在原生会话前遇到控制通道 `EAGAIN`，0 次产品输入，私有认证副本与隧道清理确认，但未取得密封退出证明。将同提交签名产物暂存系统 `/private/tmp` 后，0 输入技能目录握手再次通过；随后默认 leader 的 1 次真实输入／接收、1 次精确只读审批、1 次上下文读取、3 条原生工具事件、最终历史与随机标记匹配、原生退出及隔离清理全部通过。公开 JSON／NDJSON 可解析，无符号链接及常见凭据／邮箱命中，临时认证副本已删，原登录文件未变。此证据仍是 macOS test-only 候选：正式产品门禁、GUI、当前默认 `1.0.41`、同提交跨平台全范围和完整 Goal 均未验收。
- [receipt114](validation/cross-platform-preflight-114-4b12091-grok-unix-test-gate.safe.json)：聚焦 [run 35821695492](https://github.com/Infinimesh-ai/InfiniShell-Desktop/actions/runs/35821695492) 精确绑定 `4b12091e…` 且启用 Windows 严格原子诊断；Linux 34 步成功／2 跳过，Windows 39 步成功／1 失败／8 跳过。Windows 首败在生命周期步骤的 libtest 编译：Grok 单技能测试引用仅 Unix 编译的探针／字段却自身未标 Unix-only，报 E0433／E0609；该步骤没有执行 Windows 生命周期测试，后续严格原子 image-debug 与生产通知安装器均跳过，不得视作 receipt112 夹具已复验。5 个 artifact／25 个文件已在仓库外下载，JSON／NDJSON 可解析、无符号链接及常见凭据／邮箱命中；安全收据仅保留汇总与相对清单摘要。当前修复候选仅为该测试加 `#[cfg(unix)]`，须新提交远端实测。此轮无用户可见文案变动，无需本地化资源变更。
