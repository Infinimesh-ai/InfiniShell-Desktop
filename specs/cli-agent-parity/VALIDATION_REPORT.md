...
- [receipt110](validation/macos-working-tree-110-grok-1040-selected-skill-gate-failed.safe.json)：Grok `1.0.40` 默认入口单技能 test-only 候选经新登录重试，原生 initialize、authenticate 和 `session/new` 已通过；但输入前必需的 `available_commands_update` 未在 30 秒门限内出现，触发 `request_timed_out`。0 次产品输入／接受、0 次审批／工具事件，私有认证副本与隔离隧道清理确认；技能目录与组合均未验收，产品门禁继续关闭。工作树候选收据不能冒充干净提交或跨平台通过。
