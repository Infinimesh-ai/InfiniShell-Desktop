...
> **Mac 已验范围回顾**：所列模式的产品链和相关平台补验已有通过证据；图片格式、技能组合、普通终端输入等缺项仍在[统一清单](KNOWN_GAPS.md)，见[固定版本验收表](ACCEPTANCE_FIXED_VERSIONS_20260924.md)。下文记录 Mac 各实际构建批次，不把它们改写为最终提交二进制实测。
