      ...
      packages = forAllSystems (
        ...
        let
          ...
          warp-terminal-experimental = rustPlatform.buildRustPackage {
            ...
            env = {
              APPIMAGE_NAME = "InfiniShell-${pkgs.stdenv.hostPlatform.parsed.cpu.name}.AppImage";
            ...
            postInstall =
              ...
              in
              ''
                install -Dm755 "$out/bin/infinishell" "${installDir}/infinishell"
                rm -f "$out/bin/infinishell"
                ...
                install -Dm644 app/channels/oss/dev.infinishell.InfiniShell.desktop \
                  "$out/share/applications/dev.infinishell.InfiniShell.desktop"
                substituteInPlace "$out/share/applications/dev.infinishell.InfiniShell.desktop" \
                  --replace-fail "Exec=infinishell %U" "Exec=warp-terminal-experimental %U"

                for size in 16x16 32x32 64x64 128x128 256x256 512x512; do
                  icon="app/channels/oss/icon/no-padding/$size.png"
                  if [ -f "$icon" ]; then
                    install -Dm644 "$icon" \
                      "$out/share/icons/hicolor/$size/apps/dev.infinishell.InfiniShell.png"
                  fi
                done

                wrapProgram "${installDir}/infinishell" \
                  --prefix LD_LIBRARY_PATH : "${libraryPath}" \
                  --prefix PATH : "${executablePath}"

                mkdir -p "$out/bin"
                ln -s "${installDir}/infinishell" "$out/bin/infinishell"
                ln -s "${installDir}/infinishell" "$out/bin/warp-terminal-experimental"
              '';

            postFixup = lib.optionalString pkgs.stdenv.isLinux ''
              wrapped="/opt/warpdotdev/warp-terminal-experimental/.infinishell-wrapped"
