...
// Zap:`crate::server::server_api`(云端 API 网关)已删除,task 取消不再走远端。
// Zap:上游的 session_sharing_protocol crate 已被本地 shared_session::protocol 取代。
...
impl AgentSource {
    ...
    pub fn is_user_initiated(&self) -> bool {
        match self {
            ...
            // Cloud Mode 会话由 cloud-mode 入口自动发起,不算用户直接发起的对话。
            | AgentSource::CloudMode => false,
        }
    }
}
...
///
/// Zap:上游经云端 `ai_client.cancel_ambient_agent_task` 取消远端 task。云端 API 网关
/// (`crate::server::server_api`)已删除,Zap 里 ambient task 只有本地 id、没有远端运行体,
/// 真正的停止走 `stop_local_agent_conversation`。这里保留签名做成 no-op,避免调用点分叉。
pub fn cancel_task_with_toast<V: View>(task_id: AmbientAgentTaskId, _ctx: &mut ViewContext<V>) {
    log::warn!("cancel_task_with_toast: 云端任务取消已下线,忽略 task {task_id}");
}

/// Cancel an ambient agent task without surfacing a toast to the user.
///
/// Zap:同 [`cancel_task_with_toast`],云端取消链路已删除,保留签名做成 no-op。
pub fn cancel_task_silently<V: View>(task_id: AmbientAgentTaskId, _ctx: &mut ViewContext<V>) {
    log::warn!("cancel_task_silently: 云端任务取消已下线,忽略 task {task_id}");
}
