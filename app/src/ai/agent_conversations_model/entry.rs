...
pub enum AgentConversationProvenance {
    LocalInteractive,
    AmbientRun,
    /// Zap 没有云端会话同步,投影层不会再产出这个分类;仅保留给
    /// 显式构造条目的调用点(例如图标渲染测试)与历史数据兼容。
...
/// Zap 的会话元数据没有云端 creator/uid 字段(本地优先,没有服务端账号体系),
/// 因此本地会话的创建者恒为当前用户。
...
fn entry_for_conversation_parts(
    ...
    // Zap 的会话历史是纯本地存储:没有云端会话同步,所以会话条目永远是本地来源。
