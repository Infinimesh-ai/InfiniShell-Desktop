...
use crate::server::retry_strategies::{HttpStatusError, is_auth_error, is_transient_http_error};
...
impl AgentEventSource for AgentEventStreamClientEventSource {
    async fn open_stream(
        ...
        let stream = match filter {
            ...
            // Zap 没有云端 `?ancestor_run_id=` 端点,本地 driver 只支持显式 run_ids 过滤。
            AgentEventFilter::AncestorRunId {
                ancestor_run_id, ..
            } => {
                return Err(anyhow!(
                    "Ancestor-scoped agent event streams are not available in InfiniShell \
