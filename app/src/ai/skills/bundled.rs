...
impl BundledSkills {
    ...
    /// 按 skill name(而非 bundled id)在本地目录中查找,供 BYOP `read_skill`
    /// 按名字解析 skill 时兜底。
...
impl BundledSkill {
    ...
    /// 按 frontmatter 的 `name` 字段查找 bundled skill(id 与 name 不一定相同)。
...
pub(crate) fn build_bundled_skill_context(
    ...
    [
        // Zap 去云端后没有可供 bundled skill 调用的服务端。
        // `ChannelState::server_root_url()` 在生产下是禁用哨兵、在 test-util 下是
        // mock server 地址,两者都不该被替换进 skill 正文,所以这里固定为空串。
