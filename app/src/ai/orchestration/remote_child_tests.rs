...
// Zap:云端 `server::server_api` 网关已删除,错误类型改从本地 `ai::api_error` 取;
// 云端专属的 `CloudAgentCapacityError` 已随之下线,对应断言一并移除。
...
fn prepared_remote_request_matches_gui_wire_semantics() {
    App::test((), |mut app| async move {
        ...
        let request = StartAgentRequest {
            ...
            allow_local_child_messages: false,
        };
        app.read(|ctx| {
            ...
            // 本地 `SpawnAgentRequest::prompt` 是 `String`(非 `Option<String>`)。
            assert_eq!(prepared.spawn_request.prompt, "Inspect the code");
            assert_eq!(
                prepared.spawn_request.parent_run_id.as_deref(),
                Some("parent-run")
            );
            // 云端字段 `mode` / `agent_identity_uid` 只服务于已删除的远端 run 接口,
            // 本地 `SpawnAgentRequest` 不带它们,对应断言移除。
...
#[test]
fn quota_and_fallback_errors_keep_their_semantics() {
