...
pub fn environment_snapshot(state: &OrchestrationConfigState, _ctx: &AppContext) -> OptionSnapshot {
    // Zap:环境列表原本来自 `ai::cloud_environments::CloudAmbientAgentEnvironment`
    // (云端 ambient agent 环境对象),该模块已随云端链路整体下线。本地优先分支没有
    // 云端环境可选,这里保留函数签名并返回"仅空环境"一项,picker 依旧能正常渲染。
    let sorted_envs: Vec<(String, String)> = Vec::new();
    let current = match &state.execution_mode {
