...
fn initialize_app(app: &mut App, workspaces: Vec<Workspace>) {
    ...
    // Zap:`UserWorkspaces::mock` 不再需要云端 team / workspace client。
    app.add_singleton_model(|ctx| UserWorkspaces::mock(workspaces, ctx));
    // Zap:`ServerApiProvider` 已剥离,托管密钥客户端与生产代码一样走本地禁用实现。
    app.add_singleton_model(|_| {
        ManagedSecretManager::new(
            Arc::new(crate::local_managed_secrets::DisabledManagedSecretsClient),
            auth_state,
        )
    });
    app.update(|ctx| {
