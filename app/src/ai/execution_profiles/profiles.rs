...
use super::{
    AIExecutionProfile, AIExecutionProfileObjectModel, ActionPermission, ExecutionProfileId,
...
use crate::{LaunchMode, ObjectStoreModel, TelemetryEvent, send_telemetry_from_ctx};
...
impl AIExecutionProfileInfo {
    ...
    /// The InfiniShell Drive sync ID of this profile, if it has been synced.
    #[cfg_attr(target_family = "wasm", allow(dead_code))]
...
impl AIExecutionProfilesModel {
    ...
    pub(crate) fn new(launch_mode: &LaunchMode, ctx: &mut ModelContext<Self>) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "agent_mode_evals")] {
                ...
                let (
                    ...
                    // mappings from the currently loaded object store objects.
                    ...
                    // Insert all non-default profiles from the object store
                    for profile_object in all_profile_objects.iter().filter(|p| !p.model().string_model.is_default_profile) {
                        let profile_id = source.legacy_profile_id(profile_object.id, false);
        ...
        if uses_file_backed_profiles {
            ...
            // Eval builds never import legacy profiles into settings.
            //
            // Zap(本地化):没有账号登录,也没有云端 preferences syncer,导入只需跟随本地
            // object store 的恢复进度重试。
            #[cfg(not(feature = "agent_mode_evals"))]
            if imports_legacy_profiles && ctx.has_singleton_model::<ObjectStoreModel>() {
                ctx.subscribe_to_model(&ObjectStoreModel::handle(ctx), |me, _, event, ctx| {
                    if !me.settings_are_authoritative() {
                        me.handle_object_store_event(event, ctx);
                    }
                    if matches!(event, ObjectStoreEvent::InitialLoadCompleted) {
        ...
        // The object store may finish restoring before this model is registered. In that case its
        // one-shot event cannot reach this subscription, so run migration from the already-completed
        // state. Eval builds never import legacy profiles into settings.
        #[cfg(not(feature = "agent_mode_evals"))]
        if uses_file_backed_profiles && imports_legacy_profiles && Self::object_store_is_ready(ctx)
        {
    ...
    /// Returns whether a legacy profile object belongs to the current personal drive.
    ...
    /// Zap(本地化):本地 object store 是否已经完成初始恢复。
    ///
    /// 上游在这里等待云端 preferences 同步;Zap 没有账号与云端 preferences,只需要保证本地
    /// object store 已经恢复完毕,避免用不完整的快照覆盖已有 profile。没有 object store 的
    /// 运行环境(例如部分测试)不存在待导入的旧对象,直接视为就绪。
    fn object_store_is_ready(ctx: &AppContext) -> bool {
        !ctx.has_singleton_model::<ObjectStoreModel>()
            || ObjectStoreModel::as_ref(ctx).initial_load_completed()
    }

    /// Zap(本地化):读取本地 object store 中属于当前用户的旧版 profile 对象。
    ///
    /// 只有旧对象导入路径会用到,eval 构建里编译掉。
    #[cfg(not(feature = "agent_mode_evals"))]
    fn owned_legacy_profile_objects<'a>(
        ctx: &'a AppContext,
    ) -> Vec<&'a super::AIExecutionProfileObject> {
        if !ctx.has_singleton_model::<ObjectStoreModel>() {
            return Vec::new();
        }
    ...
    ///
    /// Returns `false` without changing authority when the legacy objects are still loading or when
    ...
    fn activate_pending_settings_collection(
        ...
        if self.source.imports_legacy_profiles() && !Self::object_store_is_ready(ctx) {
            return false;
        }
    ...
    /// Attempts to make the file-backed execution-profile collection authoritative.
    ///
    /// Zap(本地化):上游在这里等待登录与云端 preferences 的初始加载方向;Zap 只需要等待本地
    /// object store 恢复完毕,然后把本地旧 profile 对象一次性导入 [`AISettings`]。前置条件没
    /// 满足时保持 pending,由后续的就绪事件重试。
    ///
    ...
    pub(crate) fn migrate_settings_profiles(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        if !Self::object_store_is_ready(ctx) {
            return;
        }
        ...
        // Zap(本地化):本地对象没有服务端 ID,非默认 profile 的键由 `legacy_profile_id`
        // 生成并在写入设置后固定下来。
        let source = self.source;
        let mut legacy_profiles = Self::owned_legacy_profile_objects(ctx)
            .into_iter()
            .map(|profile| {
                let is_default_profile = profile.model().string_model.is_default_profile;
                let id = source.legacy_profile_id(profile.id, is_default_profile);
    ...
    pub fn get_profile_id_by_sync_id(
        ...
        if self.settings_are_authoritative() {
            ...
            let legacy_profile = ObjectStoreModel::as_ref(ctx)
                .get_object_of_type::<GenericStringObjectId, AIExecutionProfileObjectModel>(
                    sync_id,
                );
    ...
    fn handle_ai_execution_profile_created(
        ...
        let Some(object) = object_store_model
            ...
            log::warn!(
                "Received ObjectCreated event for AI execution profile but object not found in ObjectStoreModel: {sync_id:?}"
            );
            return;
        };
