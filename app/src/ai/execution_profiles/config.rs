...
struct ExecutionProfileFile {
    ...
    #[schemars(description = "Optional conversation-title model override.")]
    title_model: Option<String>,
    #[schemars(
        description = "Optional model override for proactive AI (prompt suggestions / NLD / relevant files)."
    )]
    active_ai_model: Option<String>,
    #[schemars(description = "Optional Next Command (inline suggestion) model override.")]
    next_command_model: Option<String>,
    #[schemars(
...
impl From<&AIExecutionProfile> for ExecutionProfileFile {
    fn from(profile: &AIExecutionProfile) -> Self {
        Self {
            ...
            title_model: profile.title_model.clone().map(Into::into),
            active_ai_model: profile.active_ai_model.clone().map(Into::into),
            next_command_model: profile.next_command_model.clone().map(Into::into),
...
impl TryFrom<ExecutionProfileFile> for AIExecutionProfile {
    ...
    fn try_from(file: ExecutionProfileFile) -> Result<Self, Self::Error> {
        ...
        Ok(AIExecutionProfile {
            ...
            title_model: file.title_model.map(LLMId::from),
            active_ai_model: file.active_ai_model.map(LLMId::from),
            next_command_model: file.next_command_model.map(LLMId::from),
