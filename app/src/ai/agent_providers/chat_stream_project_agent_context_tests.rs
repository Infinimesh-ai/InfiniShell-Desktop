use super::*;
use crate::ai::project_agent_context::{ProjectAgentContext, ProjectContextEntry, ProjectHostInfo};
...
fn render_project_context_block_none_and_empty() {
    ...
    assert_eq!(
        render_project_context_block(Some(&ProjectAgentContext {
            projects: Vec::new(),
            current_host_node_id: "node-1".to_owned(),
        })),
        None
    );
}
...
fn render_project_context_block_full_entry_escapes_xml() {
    let context = ProjectAgentContext {
        projects: vec![ProjectContextEntry {
            ...
            name: "商城 <\"后端\">".to_owned(),
            git_url: Some("git@github.com:acme/shop.git".to_owned()),
    ...
    assert_eq!(
        ...
        "\n\n<project_context project=\"商城 &lt;&quot;后端&quot;&gt;\">\n  \
         ...
         仓库: git@github.com:acme/shop.git\n  \
