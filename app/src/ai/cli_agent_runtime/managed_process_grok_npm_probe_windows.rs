//! Windows Grok npm 公共 cmd/PowerShell 入口的 AppContainer 版本探针。
...
#[path = "../../terminal/cli_agent_updates/sources_npm_grok_windows_contract.rs"]
mod contract;
...
impl ProbeInputs {
    ...
    pub(crate) fn canonical_stage(&self) -> &Path {
        self.arguments.get(4).map_or(Path::new(""), Path::new)
    }
    pub(crate) fn version_stage(&self) -> &Path {
        self.arguments.get(5).map_or(Path::new(""), Path::new)
    }
...
fn invalid() -> io::Error {
    io::Error::other("Grok Windows npm 探针闭包不匹配")
}
...
pub(super) fn valid_entry(program: &Path, arguments: &[OsString]) -> bool {
    if !cfg!(target_arch = "x86_64") || arguments.len() != 6 {
        return false;
    }
    ...
    .all(safe_path)
        ...
        && stage.parent() == Some(prefix.join("node_modules/@xai-official").as_path())
        && native_paths(stage, Path::new(&arguments[4]), Path::new(&arguments[5]))
}

fn native_paths(stage: &Path, canonical: &Path, versioned: &Path) -> bool {
    let Some(id) = stage
        .file_name()
        .and_then(OsStr::to_str)
        .and_then(|value| value.strip_prefix(".infinishell-npm-"))
    else {
        return false;
    };
    canonical.parent() == versioned.parent()
        && canonical
            .file_name()
            .is_some_and(|name| name == OsStr::new(&format!(".infinishell-grok-current-{id}.exe")))
        && versioned
            .file_name()
            .is_some_and(|name| name == OsStr::new(&format!(".infinishell-grok-version-{id}.exe")))
        && canonical.parent().and_then(Path::file_name) == Some(OsStr::new("bin"))
}
...
pub(super) fn validate(input: &ProbeInputs) -> io::Result<()> {
    ...
    let mut required = contract::files(contract::VERSION)?;
    ...
    let mut native_paths = BTreeSet::from([
        input.canonical_stage().to_owned(),
        input.version_stage().to_owned(),
    ]);
    for file in &input.files {
        ...
        if native_paths.remove(&file.path) {
            if file.size != native.0 || file.sha256 != native.1 {
                return Err(invalid());
            }
            continue;
        }
    ...
    if !required.is_empty() || !shims.is_empty() || !external.is_empty() || !native_paths.is_empty()
    {
        return Err(invalid());
    }
    Ok(())
...
pub(super) fn verify_external(input: &ProbeInputs) -> io::Result<()> {
    ...
    let files: Vec<_> = input
        ...
        .iter()
        .filter(|file| {
            !file.path.starts_with(input.stage())
                && file.path != input.canonical_stage()
                && file.path != input.version_stage()
        })
        .cloned()
        .collect();
...
pub(crate) fn capture(
    ...
    let arguments = vec![
        ...
        canonical_stage.as_os_str().to_owned(),
        version_stage.as_os_str().to_owned(),
    ];
    ...
    for path in contract::files(contract::VERSION)?.into_keys() {
        files.push(ExpectedFileIdentity::capture(&stage.join(path))?);
    }
    files.push(ExpectedFileIdentity::capture(canonical_stage)?);
    files.push(ExpectedFileIdentity::capture(version_stage)?);
...
pub(super) fn execute(manifest: &Manifest, record_directory: &Path) -> io::Result<()> {
    ...
    let package = install.join("node_modules/@xai-official/grok");
    let grok_home = manifest.cwd.join("config");
    let native_bin = grok_home.join("bin");
    let mut directories =
        BTreeSet::from([manifest.cwd.clone(), install.clone(), grok_home.clone()]);
    ...
    for expected in &input.files {
        ...
        } else if expected.path == input.canonical_stage() {
            native_bin.join("grok.exe")
        } else if expected.path == input.version_stage() {
            native_bin.join(format!("grok-{}.exe", contract::VERSION))
    ...
    environment.push(("GROK_HOME".into(), grok_home.into_os_string()));
