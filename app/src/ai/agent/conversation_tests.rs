...
/// ToolCall 消息但不带具体 tool payload(与上游四参 `tool_call_message` 区分)。
fn tool_call_message_without_tool(id: &str, call_id: &str) -> api::Message {
    api::Message {
        fetched_memories: vec![],
        id: id.to_string(),
        task_id: "root-task".to_string(),
...
fn initialize_custom_endpoint_usage_test_app(app: &mut App) {
    ...
    // Zap 已删除 `server::server_api`(云端 API 网关),不再注册 ServerApiProvider 单例;
    // 本组测试只覆盖自定义 endpoint 的本地用量聚合,不依赖云端网关。
    app.add_singleton_model(|_| NetworkStatus::new());
    app.add_singleton_model(UserWorkspaces::default_mock);
    app.add_singleton_model(|_| AuthStateProvider::new_for_test());
    app.add_singleton_model(AuthManager::new_for_test);
    // `LLMPreferences::new`(各用例自行注册)在构造期就取 `AgentProviderSecrets`
    // ——Zap 的 BYOP 密钥存储。
...
fn restored_conversation_does_not_infer_legacy_repair_for_unexplained_gap() {
    let conversation = AIConversation::new_restored(
        ...
        vec![api::Task {
            id: "root-task".to_string(),
            messages: vec![tool_call_message_without_tool("assistant-1", "call-1")],
            dependencies: None,
            description: String::new(),
            summary: String::new(),
            server_data: String::new(),
