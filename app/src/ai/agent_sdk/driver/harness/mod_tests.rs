...
use super::{HarnessKind, auth_check_command_for, harness_kind, validate_cli_installed};
...
#[test]
fn codex_maps_to_unsupported_harness_kind() {
    // Zap 只在本地子 pane 直启 Codex,driver 侧不提供 Codex harness runner。
    assert!(matches!(
        harness_kind(Harness::Codex),
        Ok(HarnessKind::Unsupported(Harness::Codex))
    ));
    assert!(auth_check_command_for(Harness::Codex).is_none());
}
