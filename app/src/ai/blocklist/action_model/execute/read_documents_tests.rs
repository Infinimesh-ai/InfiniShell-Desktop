...
// Zap 命名体系:上游的 `CloudModel` / `CloudObject*` / `CloudNotebook*`
// 在我方分别叫 `ObjectStoreModel` / `StoredObject*` / `NotebookObject*`。
use crate::cloud_object::model::persistence::ObjectStoreModel;
use crate::cloud_object::{
    Owner, StoredObjectMetadata, StoredObjectPermissions, StoredObjectStatuses,
    StoredObjectSyncStatus,
};
...
fn add_saved_plan_notebook(app: &mut App, document_id: AIDocumentId, content: &str) {
    ...
    ObjectStoreModel::handle(app).update(app, |object_store, _| {
        object_store.add_object(sync_id, notebook);
    });
}
