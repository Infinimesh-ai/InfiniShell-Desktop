...
use crate::settings::{
    ...
    OrchestrationMessageDisplayMode, SelectionSettings,
};
...
/// Zap:上游会从 `ServerAIConversationMetadata.creator` / `.metadata.creator_uid`
/// 解析"这条会话是谁开的"(团队共享会话里别人的头像),我方
/// `ServerAIConversationMetadata` 是纯本地结构,没有 creator 相关字段,
/// `UserProfiles` 云端目录也未引入。会话必然属于当前本机用户,直接用当前用户头像。
fn user_avatar_info_for_ai_block(
    _model: &dyn AIBlockModel<View = AIBlock>,
    app: &AppContext,
) -> UserAvatarInfo {
    current_user_avatar_info(app)
}
...
/// Zap:上游在这里为 `AIAgentActionType::SendMessageToAgent`(agent 之间互发消息)
/// 预置折叠状态。我方 `crates/ai` 的 action 枚举没有该变体 —— 编排统一走
/// `RunAgents`,收到的消息落在 `MessagesReceivedFromAgents` 分支处理 ——
/// 因此没有需要预置折叠状态的 action。保留函数壳以免调用点结构改动。
fn default_collapsible_state_for_orchestration_action(
    _action: &AIAgentActionType,
    _display_mode: OrchestrationMessageDisplayMode,
...
pub struct AIBlock {
    ...
    // Zap:上游在这里维护 UseComputer / StopRecording 两类动作的按钮组件与
    // `has_recording_related_actions` 标志。Computer Use 与屏幕录制动作已在 `ai` crate
    // 层面移除(`AIAgentActionType` 不再有 StartRecording/StopRecording/UseComputer),
    // 相关字段一并删除。
...
/// Zap:上游用 `ChannelState::oz_root_url()` 拼出 Oz 云端 run 页面的
/// `/runs/{task_id}?artifact=...` 链接来查看录屏产物。Oz 云端整体下线后
/// `ChannelState` 不再有 `oz_root_url`,本地也不存在可打开的 run 页面,
/// 因此固定返回 `None`,调用方直接走"无法打开"的提示分支。
...
impl AIBlock {
    ...
    pub fn new(
        ...
        if let Some(ambient_agent_view_model) = ambient_agent_view_model.as_ref() {
            ctx.subscribe_to_model(ambient_agent_view_model, |_, _, event, ctx| match event {
                // Zap:上游还有 `FollowupDispatched` / `ExecutionSessionReady`
                //(Cloud Mode 跟进 + 云端执行会话就绪),我方
                // `AmbientAgentViewModelEvent` 没有这两个变体。
    ...
    fn fetch_conversation_search_agent_run_titles(
        ...
        for task_id in Self::conversation_search_agent_run_ids(output) {
            // Zap:上游这里会异步向云端补拉任务数据以拿到真实标题。我方
            // `get_or_async_fetch_task_data` 只做本地查表(不收 ctx、不发请求),
            // 查不到就继续用 agent-run 兜底标题。
            let _ = AgentConversationsModel::as_ref(ctx).get_or_async_fetch_task_data(&task_id);
        }
    }
    ...
    fn finish_orchestration_message_collapsible_states(
        ...
        for message in &output.messages {
            match &message.message {
                // Zap:上游还有一条 `Action(SendMessageToAgent)` 分支(agent 之间互发
                // 消息的正文折叠)。我方 action 枚举没有该变体,统一落到下方的
                // `Action(_)` 兜底分支。
                AIAgentOutputMessageType::MessagesReceivedFromAgents { messages } => {
                    for received_message in messages {
                        let collapsible_id =
    ...
    fn handle_complete_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        // Zap:上游这里还有一段针对 `is_get_relevant_files()`(RAG 代码库检索)的
        // speedbump 逻辑。该 action 随 GetRelevantFiles 一起下线,循环永远跑不到,
        // 故整段删除。
        //
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIBlockAction::OpenRecordingArtifact { artifact_uid } => {
                ...
                // Zap:上游的兜底路径要用云端 AI client(`ServerApiProvider`)去换一个
                // 签名下载链接,该链路已随账号/云端剥离一起删除。拿不到 run 页面 URL 时
                // 只能直接提示失败。
                log::warn!(
                    "Failed to prepare recording artifact {artifact_uid}: no run page URL available"
                );
                let window_id = ctx.window_id();
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
            ...
            AIBlockAction::ViewScreenshot { action_id: _ } => {
                // Zap:Computer Use 已被移除(`AIAgentActionType::UseComputer` /
                // `UseComputerResult` 不再存在),screenshot lightbox 无从取图,
                // 这里保留 action 变体但不做任何事。
            }
