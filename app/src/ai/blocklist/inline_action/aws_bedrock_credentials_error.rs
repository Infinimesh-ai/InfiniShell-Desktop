...
impl AwsBedrockCredentialsErrorView {
    pub fn new(
        ...
        let run_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::t!("ai-aws-refresh-credentials"), PrimaryTheme)
        ...
        let configure_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(crate::t!("common-configure"), NakedTheme)
                .with_size(ButtonSize::InlineActionHeader)
                .on_click(|ctx| {
...
impl View for AwsBedrockCredentialsErrorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let make_alert_text = || {
            Text::new(
                crate::t!("ai-aws-credentials-expired-title"),
                appearance.ui_font_family(),
                14.,
            )
        ...
        let make_detail_text = || {
            Text::new(
                crate::t!(
                    "ai-aws-authentication-failed-description",
        ...
        let make_checkbox_row = || {
            ...
            let checkbox_label = Text::new(
                crate::t!("ai-aws-always-run-automatically"),
                appearance.ui_font_family(),
