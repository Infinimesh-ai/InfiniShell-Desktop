...
// Zap:`server::experiments` 只导出 `ServerExperiments`,事件类型未以
// `ServerExperimentsEvent` 之名重导出;`server::server_api` 云端网关已删除。
...
impl RunAgentsCardView {
    pub fn new(
        ...
        ctx.subscribe_to_model(&ServerExperiments::handle(ctx), |me, _, event, ctx| {
            // `ServerExperiments` 只有 `ExperimentsUpdated` 一个事件。
            let _ = event;
    ...
    /// Fetches available runners for the Runner picker.
    ///
    /// Zap:上游经云端 `ServerApiProvider::get_factory_client()` 的 `getRunners`
    /// (warp_graphql)拉取远端 runner 列表。云端 API 网关与 `warp_graphql` 均已删除,
    /// Zap 只有本地执行,没有可选 runner。这里保留函数与调用点,做成 no-op:
    /// `self.runners` 保持为空,picker 渲染为空列表。
    fn fetch_runners(&mut self, _ctx: &mut ViewContext<Self>) {
        self.runners_loading = false;
    }
