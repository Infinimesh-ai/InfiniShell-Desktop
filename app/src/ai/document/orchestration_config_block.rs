...
impl OrchestrationConfigBlockView {
    pub fn new(
        ...
        ctx.subscribe_to_model(&ServerExperiments::handle(ctx), |me, _, event, ctx| {
            // `ServerExperiments` 只有 `ExperimentsUpdated` 一个事件,且其事件类型
            // (`server::experiments::model::Event`)未从模块对外导出,这里直接忽略。
            let _ = event;
    ...
    /// 上游此处通过云端 `getRunners` GraphQL 拉取远程 runner 列表。Zap 是本地优先
    /// fork,云端 API 网关(`server::server_api` / `warp_graphql`)已整体删除,
    /// 因此这里退化为 no-op:runner 列表恒为空,picker 会显示"无可用 runner"。
    /// 保留函数与调用点,便于日后接回本地/自托管 runner 来源。
    fn fetch_runners(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(handle) = self.pickers.runner_picker.clone() {
            oc::populate_runner_picker(&handle, &self.runners, &current, false, ctx);
        }
        ctx.notify();
    }
