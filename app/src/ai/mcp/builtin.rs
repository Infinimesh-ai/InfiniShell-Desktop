...
///
/// Zap:上游在此额外检查 Firebase 托管 token 是否临近过期(2 分钟内到期即视为
/// 不可用,避免连接 preflight 401 把内置 server 误导进交互式 MCP OAuth 流程)。
/// 本地化后的 `crate::auth::Credentials` 只剩 `ApiKey` / `Test` 两态,没有
/// Firebase 托管 token,也就没有过期竞态,该检查随之删除。
