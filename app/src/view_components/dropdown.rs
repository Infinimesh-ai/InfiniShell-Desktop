...
pub trait DropdownItemAction: Action {
    ...
    /// Zap:克隆为 `Box<dyn Any>` 以便还原具体类型。
    /// `select_action_and_close` 用它把已擦除的 item action 还原成具体类型后
    /// 走延迟派发,避免选中项时重入更新崩溃(#262)。
...
pub struct Dropdown<A: DropdownItemAction = ()> {
    ...
    vertical_margin: f32,
    /// Zap:None 时按 Appearance 的 UI 字体自适应顶栏高度(见 render_top_bar)。
    ...
    A: DropdownItemAction,
{
    ...
    fn select_action_and_close(
        ...
        // Zap:先关闭再延迟派发,避免 action 处理方重入更新本视图导致崩溃(#262)。
        // 类型化 item(`set_items` 等)可以还原出具体类型 `A` 走延迟队列;
        // `set_rich_items` 注入的异构擦除 action 无法安全还原具体类型,
        // 保持上游的立即派发行为。
        self.close(ctx);
        if let Ok(action) = action.clone_any().downcast::<A>() {
