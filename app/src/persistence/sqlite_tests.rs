...
use super::{
    ...
    start_writer, to_cloud_object_permissions,
};
use crate::app_state::{
...
// Zap:codebase 向量索引整条链路(`ai::index::full_source_code_embedding` /
// `PersistedData::codebase_indices` / `ModelEvent::{Upsert,Delete}CodebaseIndexMetadata` /
// `save_codebase_index_metadata`)已下线,对应的种子数据与断言随之删除。
// 只保留仍然存在的行为:app state 的持久化往返、TUI 数据域的读写。
#[test]
...
fn tui_database_in_tui_subdirectory_round_trips_data() {
    ...
    writer
        ...
        .send(ModelEvent::UpsertUserProfiles {
            profiles: vec![UserProfileWithUID {
                local_user_uid: UserUid::new("creator-uid"),
    ...
    // ...but restores command history and shared data like creator profiles.
