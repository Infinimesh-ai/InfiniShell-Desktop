        ...
        let find_box = Flex::row()
            ...
            .with_children([
                ...
                self.render_toggle_button(
                    ...
                    &crate::t!("find-case-sensitive-tooltip"),
        ...
        let text = match action {
            FindBarAction::ToggleRegex => {
                if self.searcher.as_ref(ctx).is_regex() {
                    crate::t!("notebook-a11y-enable-regex-search")
                } else {
                    crate::t!("notebook-a11y-disable-regex-search")
                }
            }
            FindBarAction::ToggleCaseSensitive => {
                if self.searcher.as_ref(ctx).is_case_sensitive() {
                    crate::t!("notebook-a11y-enable-case-sensitive-search")
                } else {
                    crate::t!("notebook-a11y-disable-case-sensitive-search")
                }
            }
            FindBarAction::FocusNextMatch => crate::t!("notebook-a11y-focus-next-match"),
            FindBarAction::FocusPreviousMatch => {
                crate::t!("notebook-a11y-focus-previous-match")
            }
            FindBarAction::Close => crate::t!("notebook-a11y-close-find-bar"),
        };
