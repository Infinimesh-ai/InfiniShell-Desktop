...
pub fn initialize_app(app: &mut App) {
    ...
    // OrchestrationEventService unconditionally, so it must be registered before
    // bootstrap.
    // Zap:`OrchestrationEventStreamer` 依赖已删除的云端 `server_api` SSE 网关,
    // 模块未挂载,不再注册。
    app.add_singleton_model(
    ...
    // Zap:`shared_session::permissions_manager::SessionPermissionsManager`
    // (会话共享权限网关)已随共享会话云端链路删除,不再注册。
...
fn zero_state_hint_text_only_registers_active_slash_command_placeholders() {
    App::test((), |mut app| async move {
        ...
        // Zap:上游这里用 `/continue-locally` 作为「非活跃 slash command」样本,
        // 该命令属云端 → 本地交接,已随 handoff 链路删除。改用 `/fork`
        // (`Availability::AGENT_VIEW | ACTIVE_CONVERSATION | ...`,同样带 hint_text),
        // 在裸终端里同样不活跃,断言语义不变。
        let inactive_command_prefix = format!("{} ", commands::FORK.name);

        editor.update(&mut app, |editor, ctx| {
            editor.set_placeholder_text_with_prefix(
                inactive_command_prefix.clone(),
                "stale hint",
                ctx,
            );
        });
        ...
        assert!(
            editor.read(&app, |editor, _| editor
                .placeholder_text(&inactive_command_prefix)
                .is_none()),
            "/fork should not be registered outside an agent view with an active conversation"
        );
    ...
    App::test((), |mut app| async move {
        ...
        input.update(&mut app, |input, ctx| {
            let view_model = ctx.add_model(|ctx| {
                // 上游新增 `has_parent_terminal` 形参;测试里的 terminal 没有父终端。
                AmbientAgentViewModel::new(terminal_view_id, false, ctx)
            });
...
// Zap:上游这里有三个云端跟进(cloud follow-up)附件上传测试
// (`restore_cloud_followup_input_after_upload_failure_restores_prompt`、
// `should_upload_cloud_followup_attachments_matches_cloud_mode_image_context_flag`、
// `upload_files_then_submit_cloud_followup_restores_input_on_upload_error`)。
// 附件预签名上传链路随 `server::server_api` 删除,
