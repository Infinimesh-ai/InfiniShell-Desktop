...
impl BlockList {
    ...
    fn do_clear_visible_screen(&mut self) {
        ...
        //
        // Zap 注:in-band 命令(例如 SSH subshell-bootstrap 之后远端派发的 generator)会临时
        // "借身"用户的 active block 并把它隐藏(`should_hide_block` → `height()=0`)。旧实现
        // 按 started 判断会把 gap 插到该 block 之后,导致 Ctrl+L 后 input 框被 layout 到顶部。
        // 上游改为按可见高度判断后,height=0 的借身 block 自然走 "插到之前" 分支,与我方
        // 原修复语义一致,故直接采用上游实现。
    ...
    pub fn create_restored_command_block(
        ...
        // Zap:适配上游 create_new_block 改为接收 PromptMetadata
