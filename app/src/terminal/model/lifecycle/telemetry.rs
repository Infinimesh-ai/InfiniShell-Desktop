...
///
/// Zap:遥测上报层已删除,`TelemetryEvent` / `TelemetryEventDesc` trait 与
/// `register_telemetry_event!` 宏均已不存在。这里只保留事件类型壳和 payload 构造,
/// 供 `send_telemetry_from_ctx!`(编译期 shim)的既有调用点与生命周期诊断单测继续工作。
#[derive(Debug)]
...
#[allow(dead_code)]
impl LifecycleTelemetryEvent {
    pub(in crate::terminal) fn payload(&self) -> Option<Value> {
        match self {
    ...
    pub(in crate::terminal) fn contains_ugc(&self) -> bool {
        false
    }
}
