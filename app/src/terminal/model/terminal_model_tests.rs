...
fn ambient_agent_deferred_terminal_model_starts_view_pending() {
    // Zap:上游叫 `new_for_cloud_mode_shared_session_viewer` / `is_dummy_cloud_mode_session`,
    // 我方统一命名为 ambient agent(`new_for_ambient_agent_shared_session_viewer` /
    // `is_dummy_ambient_agent_session`)。覆盖的行为不变,只跟随改名。
