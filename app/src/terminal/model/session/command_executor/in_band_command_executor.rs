            ...
            }) = running_command.as_ref()
            {
                ...
                let in_band_command = match shell.shell_type() {
                    ShellType::PowerShell => {
                        format!(
                            "InfiniShell-Run-GeneratorCommand {id} '{escaped_command}' -ErrorAction Ignore"
                        )
                    }
                    ShellType::Fish => {
...
impl CommandExecutor for InBandCommandExecutor {
    ...
    /// `warp_run_generator_command`/`InfiniShell-Run-GeneratorCommand` shell script API that is
    /// declared as part of InfiniShell's bootstrap script.
    ///
...
pub fn is_in_band_command(command: &str) -> bool {
    ...
    trimmed.starts_with("InfiniShell-Run-GeneratorCommand ")
