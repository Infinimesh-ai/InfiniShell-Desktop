    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
                ...
                match json_marker_char {
                    HEX_ENCODED_JSON_MARKER => {
                        ...
                        let Some(data_str) = params
                            ...
                            log::warn!("InfiniShell OSC marker did not contain payload");
                            return;
                        };
                        safe_debug!(
                            safe: ("Received InfiniShell OSC string for shell hook"),
                            full: ("Received InfiniShell OSC string for shell hook with JSON payload: {:?}", data_str)
                        );
            ...
            WARP_RESET_GRID_OSC_MARKER => {
                log::debug!("Received InfiniShell OSC string for reset grid");
            ...
            WARP_COMPLETIONS_OSC_MARKER => match params.get(1) {
                ...
                Some(&WARP_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "InfiniShell completions match result OSC marker did not contain payload"
                        );
                        return;
                    };
                ...
                Some(bytes) if bytes.starts_with(WARP_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "InfiniShell completions match metadata OSC marker did not contain payload"
                        );
                        return;
                    };
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            log::warn!(
                                "Invalid InfiniShell OSC marker parameter for completions match metadata: {parameter}"
                            );
                        }
                    }
                }
