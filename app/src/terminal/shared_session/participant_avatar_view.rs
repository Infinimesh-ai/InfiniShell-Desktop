...
pub enum ParticipantAvatarEvent {
    ScrollToSharedSessionParticipant { participant_id: ParticipantId },
    MenuOpened { participant_id: ParticipantId },
    MenuClosed,
}
...
pub fn render_viewer_role_button(
    ...
    let button = icon_button(appearance, icon, false, mouse_state_handle.clone())
        .with_tooltip(move || {
            ui_builder
                .tool_tip(crate::t!("terminal-shared-session-change-role"))
                .build()
                .finish()
        })
        .build()
