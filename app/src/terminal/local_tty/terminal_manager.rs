...
pub struct TerminalManager<S> {
    ...
    // Zap:上游这里还持有 `session_sharer: Rc<RefCell<Option<ModelHandle<Network>>>>`
    // ——云端 session sharing 的 sharer 侧网络句柄。
    // `terminal::shared_session::sharer` 整个模块已随云端会话共享一同物理删除,
    // 该字段没有任何可写入的来源,故移除。
}
...
impl<S> TerminalManager<S> {
    ...
    fn create_pty(
        ...
        //
        // Zap:保留 tmux warpify 流程(上游 #12478 已删,我们不跟进)。tmux 包装器取代
        // 原有的 ControlMaster 包装器,所以它生效时不能再启用 legacy wrapper。
