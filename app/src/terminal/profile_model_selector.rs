...
pub enum ProfileModelSelectorAction {
    ...
    // Zap:上游的 `SelectHarnessModel { model_id, reasoning_level }` 已随
    // 第三方 harness 模型选择器一并移除,详见 `harness_model_display_name` 处的说明。
    ManageProfiles,
    ToggleProfileMenu,
    ToggleModelMenu,
}
...
impl ProfileModelSelector {
    ...
    pub fn set_ambient_agent_view_model(
        ...
        // Zap:上游还订阅 `HarnessModelSelected` / `RunLifecycleChanged` /
        // `FollowupDispatched`。前者随 harness 模型选择器移除,后两者属云端
        // Cloud Mode 跟进流程,我方 `AmbientAgentViewModelEvent` 没有这些变体。
    ...
    // Zap:上游还有 `is_locked_for_cloud_followup`(Cloud Mode 跟进时锁定模型,
    // 因为服务端会沿用原任务的模型配置)。Cloud Mode 已整片下线,
    // `AmbientAgentViewModel::is_ready_for_cloud_followup_prompt` 不存在,故删除。
    ...
    // Zap:上游在这里还有第三方 harness(Claude Code / Codex 等)的模型选择器:
    //   `active_harness_model_info` / `harness_model_display_name` / `refresh_harness_model_menu`。
    // 它们依赖 `AmbientAgentViewModel::{selected_harness_model_id,
    //  selected_harness_reasoning_level, set_harness_model_selection}` 以及
    //  `AmbientAgentViewModelEvent::HarnessModelSelected` —— 我方的
    ...
    fn render_model_section(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Zap:Cloud Mode 跟进锁定已随该功能下线,只剩非 Oz harness 运行的静默锁定。
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            ProfileModelSelectorAction::ToggleModelMenu => {
                ...
                // Zap:3p harness 没有独立的模型菜单了(见上方说明),
                // 这里与 Oz 走同一条分支。
