...
// 我方 `SpawnAgentRequest`(`ai::ambient_agents`)没有 `mode` / `agent_identity_uid` /
// `conversation_id` / `initial_snapshot_token` / `snapshot_disabled` / `orchestration_handoff`
// 这些云端快照与交接字段,且 `prompt` 是 `String` 而非 `Option<String>`。
...
/// A promptless spawn request (empty `prompt`), modeling a handoff where the
/// agent skips its initial turn.
...
fn enter_cloud_setup_with_conversation(
    ...
    // 上游的 `enter_ambient_agent_setup` 是 CloudMode 专属包装(内部就是这一句 +
    // 可选的初始 prompt 注入),我方未保留;直接走等价的 agent view 进入路径。
    view.enter_agent_view_for_new_conversation(None, AgentViewEntryOrigin::AmbientAgent, ctx);
...
// Zap:上游的 `dispatched_cloud_followup_uses_locked_queue_row_when_v2_is_enabled`
// 覆盖的是云端 followup 派发链路(`AmbientAgentViewModel::submit_cloud_followup` +
// `AmbientAgentViewModelEvent::FollowupDispatched`),该链路随云端交接一起剥离,
// 模型上已无对应方法与事件,故删除该测试。
...
fn cloud_setup_cleanup_events_remove_the_locked_queue_row() {
    ...
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            // 我方 `HarnessCommandStarted` 不带 `block_id`;
            // `HandoffSnapshotUploadFailed`(云端快照上传失败)随交接链路一起删除。
            let cleanup_events = [
