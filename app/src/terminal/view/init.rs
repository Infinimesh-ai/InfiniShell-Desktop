...
// Zap:原 `terminal::ssh::error` 模块随上游删除 in-terminal warpification
// 流程一并下线(其依赖的 bundled/ssh 脚本资产与 ansi::WarpificationUnavailableReason
// 都已不存在),该上下文键是它唯一还被引用的部分,就近保留。
const SSH_ERROR_BLOCK_VISIBLE_KEY: &str = "SshErrorBlockVisible";
    ...
    app.register_fixed_bindings([
        FixedBinding::new_per_platform(
            ...
            TerminalAction::StartNewAgentConversation {
                // 我方 `AgentViewEntryOrigin::Keybinding` 是无载荷变体(上游才带 keystroke)。
