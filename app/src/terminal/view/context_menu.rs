...
use super::{
    ...
    ForkFromExchange, ForkedConversationDestination, MenuItem, MenuItemFields,
    ONEKEY_CONTEXT_MENU_WIDTH, RichContentLink, ServerConversationToken, ServerOutputId,
...
impl TerminalView {
    pub(super) fn ai_block_copying_menu_items(
        ...
        // Zap:AI 会话分享(复制分享链接 / 分享会话)依赖云端,已删除。
    ...
    fn conversation_menu_items(
        ...
        items.push(
            MenuItemFields::new(crate::t!("menu-ai-block-fork"))
                .with_on_select_action(TerminalAction::ContextMenu(
    ...
    pub(super) fn show_context_menu(
        ...
        ctx.update_view(&self.context_menu, |context_menu, view_ctx| {
            ...
            // OneKey / su root 密码菜单需要更宽的容器来放下条目文案。
