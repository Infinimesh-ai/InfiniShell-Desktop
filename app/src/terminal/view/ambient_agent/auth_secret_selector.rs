...
// Zap:上游从 `crate::ai::cloud_agent_settings::CloudAgentSettings` 直接读写 auth secret
// 的持久化选择。该设置组随云端 agent 链路一并未引入,统一走 `ai::orchestration` 里
// 已定稿的保留签名 no-op 门面。
use crate::ai::orchestration::{
    AuthSecretSelection, persist_auth_secret_selection, resolve_auth_secret_selection_for_harness,
};
...
impl AuthSecretSelector {
    pub fn new(
        ...
        let button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                crate::t!("ambient-agent-inherit-key-from-environment"),
                NakedHeaderButtonTheme,
            )
            .with_size(ButtonSize::AgentInputButton)
            .with_menu(true)
            .with_icon(Icon::Key)
            .with_tooltip(crate::t!("ai-orchestration-api-key"))
            .on_click(|ctx| {
    ...
    fn maybe_restore_auth_secret_from_settings(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let saved_name = match resolve_auth_secret_selection_for_harness(harness.config_name(), ctx)
        {
            AuthSecretSelection::Named(name) => Some(name),
            _ => None,
        };
    ...
    fn update_sidecar_visibility_from_hover(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let is_new_item = self.menu.read(ctx, |menu, _| {
            menu.items()
                ...
                .map(|item| {
                    matches!(item,
                    MenuItem::Item(fields) if fields.label() == crate::t!("common-new"))
                })
                .unwrap_or(false)
        });
        if is_new_item {
    ...
    fn refresh_button(&mut self, ctx: &mut ViewContext<Self>) {
        let label = self
            ...
            .selected_harness_auth_secret_name()
            .map(|s| s.to_string())
            .unwrap_or_else(|| crate::t!("ambient-agent-inherit-key-from-environment"));
    ...
    fn handle_secret_deleted(
        ...
        // Zap:上游在此把 `CloudAgentSettings.last_selected_auth_secret` 里指向刚被删除
        // 的 secret 的记录抹掉。持久化门面是 no-op(设置组未引入),这里等价地把该
        // harness 的选择重置为 Unset,行为上与"记录已被抹掉"一致。
        if resolve_auth_secret_selection_for_harness(harness.config_name(), ctx)
            == AuthSecretSelection::Named(name.clone())
        {
            persist_auth_secret_selection(harness.config_name(), &AuthSecretSelection::Unset, ctx);
        }
        ...
        if removed_pending {
            ...
            let message = crate::t!("ambient-agent-api-key-deleted", name = name.as_str());
    ...
    fn handle_secret_deletion_failed(
        ...
        if removed_pending {
            let window_id = ctx.window_id();
            let message = crate::t!(
                "ambient-agent-api-key-delete-failed",
                name = name.as_str(),
...
fn build_main_menu_items(
    ...
    let header = MenuItem::Header {
        fields: MenuItemFields::new(crate::t!("ai-orchestration-api-key"))
    ...
    items.push(MenuItem::Item(
        MenuItemFields::new(crate::t!("ambient-agent-inherit-key-from-environment"))
    ...
    match fetch_state {
        AuthSecretFetchState::Loaded(secrets) => {
            for secret in secrets {
                ...
                let fields = MenuItemFields::new(secret.name.clone())
                    ...
                    .with_right_side_icon_a11y_label(crate::t!(
                        "ambient-agent-delete-api-key-a11y",
        ...
        AuthSecretFetchState::NotFetched | AuthSecretFetchState::Loading => {
            items.push(MenuItem::Item(
                MenuItemFields::new(crate::t!("common-loading"))
                    .with_font_size_override(ITEM_FONT_SIZE)
                    .with_padding_override(ITEM_VERTICAL_PADDING, MENU_HORIZONTAL_PADDING)
                    .with_disabled(true)
                    .with_override_text_color(header_text_color),
        ...
        AuthSecretFetchState::Failed(_) => {
            items.push(MenuItem::Item(
                MenuItemFields::new(crate::t!("terminal-secrets-load-failed"))
                    .with_font_size_override(ITEM_FONT_SIZE)
                    .with_padding_override(ITEM_VERTICAL_PADDING, MENU_HORIZONTAL_PADDING)
                    .with_disabled(true)
                    .with_override_text_color(header_text_color),
    ...
    items.push(MenuItem::Item(
        MenuItemFields::new(crate::t!("common-new"))
            .with_font_size_override(ITEM_FONT_SIZE)
            .with_padding_override(ITEM_VERTICAL_PADDING, MENU_HORIZONTAL_PADDING)
            .with_override_hover_background_color(hover_background)
            .with_icon(Icon::Plus)
...
fn build_sidecar_items(
    ...
    let header = MenuItem::Header {
        fields: MenuItemFields::new(crate::t!("ambient-agent-choose-key-type"))
            .with_font_size_override(HEADER_FONT_SIZE)
            .with_override_text_color(header_text_color)
            .with_padding_override(6., MENU_HORIZONTAL_PADDING)
            .with_no_interaction_on_hover(),
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AuthSecretSelectorAction::SelectSecret(name) => {
                ...
                // Persist the selection per-harness.
                // Zap:上游还会调 `CloudAgentSettings::mark_harness_auth_ftux_completed`,
                // 该设置组未引入(FTUX 标记在别处也已跳过),故只保留选择的持久化门面。
