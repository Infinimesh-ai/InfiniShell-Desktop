...
// Zap:session-sharing-protocol crate 已剥离,SessionSourceType 由本地 protocol 模块提供。
...
fn test_begin_viewing_ambient_session_creates_and_wires_model_for_link_join_viewer() {
    ...
    App::test((), |mut app| async move {
        ...
        terminal.read(&app, |view, ctx| {
            ...
            // Zap:上游还断言 `!model.is_ready_for_cloud_followup_prompt()`(live session
            // 期间门控云端 follow-up)。云端 follow-up 链路已剥离,该方法不复存在。
        });
    });
}
...
fn test_non_owned_tombstone_is_removed_for_followup_and_reinserted_after_completion() {
    ...
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            // Zap:上游是带 session_id 的 `ExecutionSessionReady`,我方对应的变体是
            // 无载荷的 `SessionReady`(执行会话就绪信号本身保留)。
            view.handle_ambient_agent_event(
                &crate::terminal::view::ambient_agent::AmbientAgentViewModelEvent::SessionReady,
                ctx,
            );
            view.on_ambient_agent_execution_ended(ctx);
            assert!(view.conversation_ended_tombstone_view_id.is_some());
