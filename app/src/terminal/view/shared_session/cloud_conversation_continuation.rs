...
/// Zap:上游这里按云端 ACL 判定当前用户对该会话的读写权限 —— owner space、
/// link sharing、guest 列表(`ServerPermissions` / `ServerObjectGuest` /
/// `ServerGuestSubject`)逐项取最高授权。
///
/// Zap 是本地优先 fork:`crate::cloud_object` 不再导出 `ServerPermissions` /
/// `ServerGuestSubject`(app 侧只剩本地化后的 `StoredObjectPermissions` /
/// `drive::sharing::Subject`),`ServerAIConversationMetadata` 也不再携带
/// `permissions` / `metadata` 字段。会话元数据只存在于本机历史中,
/// 能读到 metadata 就等价于本人拥有该会话,因此这里退化为:
/// 未登录(拿不到 user_uid)→ `Unknown`,否则一律 `Edit`。
fn conversation_access(
    _metadata: &ServerAIConversationMetadata,
    app: &AppContext,
) -> ConversationAccess {
    if AuthStateProvider::as_ref(app).get().user_id().is_none() {
...
fn task_harness(task: &AmbientAgentTask) -> AIAgentHarness {
        ...
        .unwrap_or(Harness::Oz)
    {
        ...
        Harness::OpenCode | Harness::Grok | Harness::Unknown => AIAgentHarness::Unknown,
    }
}
