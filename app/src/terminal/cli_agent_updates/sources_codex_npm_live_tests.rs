//! 零模型、私有官方 npm 树的真实事务验收；运行已绑定 npm 的只读 prefix 查询及真实 Node 公共入口，不调用模型。
...
fn step() -> String {
    std::env::var("INFINISHELL_CLI_CODEX_NPM_UPDATE_STEP").unwrap_or_default()
}
...
pub(super) fn audit_probe(invocation: &sources::Invocation) -> Result<(), Error> {
    EVIDENCE
        .try_with(|root| {
            ...
            let version = program == package(root).join("bin/codex.js")
...
pub(super) async fn checkpoint(point: Point) {
    let _ = EVIDENCE.try_with(|root| {
        let source = super::journal_path(&sources::journal_root().unwrap(), CLIAgent::Codex);
...
const SCOPE: &str = "codex_npm_transaction_no_model_v1";
...
const SOURCES: &[(&str, &[u8])] = &[
    ...
    (
        "app/src/terminal/cli_agent_updates/sources_codex_npm_live_tests.rs",
        include_bytes!("sources_codex_npm_live_tests.rs"),
    ),
    (
    ...
    (
        "app/src/terminal/cli_agent_updates/sources_npm_codex.rs",
        include_bytes!("sources_npm_codex.rs"),
    ),
    (
        "app/src/ai/cli_agent_runtime/managed_process_npm_probe.rs",
        include_bytes!("../../ai/cli_agent_runtime/managed_process_npm_probe.rs"),
    ),
    (
        "app/src/ai/cli_agent_runtime/managed_process_npm_probe_macos.rs",
    ...
    (
        ...
        include_bytes!("../../../../script/cli-agent-parity/run_codex_npm_update_live.py"),
    ),
];
...
fn entry(root: &Path) -> PathBuf {
    root.join("prefix/bin/codex")
}
fn package(root: &Path) -> PathBuf {
    root.join("prefix/lib/node_modules/@openai/codex")
}
...
fn validate(manifest: &Manifest, path: &Path) -> Result<(), String> {
    ...
    check(
        std::env::current_dir().ok().as_ref() == Some(&root.join("project"))
            && std::env::var("INFINISHELL_CLI_CODEX_NPM_UPDATE_ALLOW").as_deref() == Ok(SCOPE)
    ...
    let allowed = env_paths()
        ...
        .chain([
            ...
            "INFINISHELL_CLI_CODEX_NPM_UPDATE_ALLOW",
            "INFINISHELL_CLI_CODEX_NPM_UPDATE_MANIFEST",
            "INFINISHELL_CLI_SUPERVISOR_EXECUTABLE",
            "INFINISHELL_CLI_CODEX_NPM_UPDATE_STEP",
...
async fn exercise(manifest: &Manifest) -> Result<Value, String> {
    ...
    let plan = prepare(manifest, "0.156.1").await?;
    check(plan.installed_version == "0.155.1", "old_version_not_fixed")?;
    ...
    if manifest.case == "updated" {
        ...
        let journal_path = super::journal_path(&journal_root, CLIAgent::Codex);
    ...
    let observed = sources::version(CLIAgent::Codex, &entry(root))
        .await
        .map_err(mapped)?;
    ...
    let candidate_probe = if manifest.case == "candidate_changed_preserved" {
        let journal: super::Journal = serde_json::from_slice(
            &fs::read(super::journal_path(&journal_root, CLIAgent::Codex))
                .map_err(|_| "candidate_journal_missing")?,
        )
        .map_err(|_| "candidate_journal_invalid")?;
        check(
            journal.probe.is_none(),
            "changed_candidate_probe_was_created",
        )?;
        let processes = journal_root.join("cli-agent-processes");
        check(
            !processes.exists()
                || fs::read_dir(processes)
                    .map_err(|_| "candidate_processes_unreadable")?
                    .next()
                    .is_none(),
            "changed_candidate_created_generation",
        )?;
        json!({"started":false,"rejected_before_spawn":true})
    } else {
        candidate_probe_evidence(manifest, &journal_root)?
    };
    Ok(
        json!({"case":manifest.case,"accepted":true,"public_version":observed,"expected_failure":expected_failure,
        "recover_invoked":recover_invoked,"entry_link_preserved":true,"candidate_probe":candidate_probe,
        "model_inputs_sent":0,"npm_readonly_prefix_query_covered":true,"npm_install_or_lifecycle_executed":false,
        "consumer_channel_discovery_covered":false,"g09_closed":false}),
    )
}

fn candidate_probe_evidence(manifest: &Manifest, journal_root: &Path) -> Result<Value, String> {
    let raw = fs::read(manifest.root.join("exchanged-journal.safe.json"))
        .map_err(|_| "exchanged_journal_missing")?;
    let journal: super::Journal =
        serde_json::from_slice(&raw).map_err(|_| "exchanged_journal_invalid")?;
    let probe = journal.probe.as_ref().ok_or("node_probe_missing")?;
    let closure = probe
        .codex_closure
        .as_ref()
        .ok_or("node_probe_closure_missing")?;
    let stage = journal
        .owner
        .package_root
        .parent()
        .ok_or("package_parent_missing")?
        .join(&journal.stage_name);
    check(
        journal.agent == "codex"
            && journal.old_version == "0.155.1"
            && journal.target_version == "0.156.1"
            && journal.phase == super::Phase::SwapIntent
            && probe.program == manifest.node.path
            && closure.arguments
                == [
                    stage.join("bin/codex.js").into_os_string(),
                    "--version".into(),
                ]
            && probe.observed_version.as_deref() == Some("0.156.1")
            && closure.expected_files.len() > 4,
        "node_public_entry_not_verified",
    )?;
    let binding = managed_process::PreparedLaunchBinding::codex_npm_version_probe(
        probe.binding_digest.clone(),
    )
    .map_err(|_| "node_binding_invalid")?;
    let receipt =
        managed_process::confirmed_exit_with_binding(journal_root, probe.generation, &binding)
            .map_err(|_| "node_cleanup_invalid")?
            .ok_or("node_cleanup_missing")?;
    check(
        receipt.cleanup_confirmed
            && receipt.exit_code == Some(0)
            && receipt.containment != "not_started",
        "node_process_not_successfully_cleaned",
    )?;
    let output = fs::read(
        manifest
            .root
            .join(format!("probe-{}.stdout", probe.generation)),
    )
    .map_err(|_| "node_stdout_missing")?;
    check(
        std::str::from_utf8(&output).ok().map(str::trim) == Some("codex-cli 0.156.1"),
        "node_stdout_version_mismatch",
    )?;
    let raw_exit = fs::read(
        journal_root
            .join("cli-agent-processes")
            .join(probe.generation.to_string())
            .join("exit.json"),
    )
    .map_err(|_| "node_raw_exit_missing")?;
    save_raw(
        &manifest
            .root
            .join(format!("node-exit-{}.safe.json", step())),
        &raw_exit,
    )?;
    Ok(
        json!({"started":true,"program":probe.program,"arguments":closure.arguments,
        "generation":probe.generation,"binding_digest":probe.binding_digest,
        "expected_files":closure.expected_files,"stdout_sha256":format!("{:x}",Sha256::digest(&output)),
        "cleanup_receipt":receipt,"public_node_launcher_executed":true}),
    )
}
...
async fn cold_recover(manifest: &Manifest) -> Result<Value, String> {
    ...
    let journal_root = sources::journal_root().map_err(mapped)?;
    let result = inspect(manifest, "0.156.1").await;
    let version;
    if manifest.case == "swap_receipt_missing" {
        let report = result.map_err(mapped)?;
        check(
            report.installed_version == "0.155.1" && report.source == Source::Npm,
            "cold_inspect_version",
        )?;
        check(
            tree(root)? == before && !super::journal_path(&journal_root, CLIAgent::Codex).exists(),
        ...
        check(
            super::journal_path(&journal_root, CLIAgent::Codex).exists(),
    ...
    let binding =
        managed_process::PreparedLaunchBinding::codex_npm_version_probe(probe.binding_digest)
    ...
    let candidate_probe = candidate_probe_evidence(manifest, &journal_root)?;
    let after = tree(root)?;
    ...
    Ok(
        ...
        "model_inputs_sent":0,"cleanup_confirmed":true,"candidate_probe":candidate_probe,
        "source_discovery_prefix_query_covered":manifest.case == "swap_receipt_missing",
        "consumer_channel_discovery_covered":false,"g09_closed":false}),
    )
}
...
async fn real_codex_npm_update_without_model() {
    let path = PathBuf::from(
        std::env::var_os("INFINISHELL_CLI_CODEX_NPM_UPDATE_MANIFEST").expect("缺少私有清单"),
    );
...
async fn fixed_npm_release_input_is_scoped_and_never_enables_future_versions() {
    assert!(sources::channel_supported(CLIAgent::Codex, Channel::Latest));
    assert!(!sources::channel_supported(CLIAgent::Codex, Channel::Stable));
    assert!(fixed_release(CLIAgent::Codex, Channel::Latest).is_none());
    RELEASE
        .scope("0.156.1".to_owned(), async {
            assert_eq!(
                fixed_release(CLIAgent::Codex, Channel::Latest).as_deref(),
                Some("0.156.1")
            );
            assert!(fixed_release(CLIAgent::Claude, Channel::Latest).is_none());
            assert!(fixed_release(CLIAgent::Codex, Channel::Stable).is_none());
        })
        .await;
    RELEASE
        .scope("0.156.2".to_owned(), async {
            assert!(fixed_release(CLIAgent::Codex, Channel::Latest).is_none());
        })
        .await;
    assert!(fixed_release(CLIAgent::Codex, Channel::Latest).is_none());
