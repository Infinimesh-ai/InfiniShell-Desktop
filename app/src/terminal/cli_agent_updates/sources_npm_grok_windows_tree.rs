...
fn member_path(root: &Path, relative: &Path) -> PathBuf {
...
pub(super) fn rename(root: &Path, destination: &Path, expected: &Snapshot) -> Result<(), Error> {
    ...
    let root_identity = expected
        .members
        .get(Path::new(""))
        .ok_or(Error::RecoveryRequired)?;
    // 单文件根的调用方持有 freeze_images 写句柄；本句柄必须与该冻结句柄共享写访问。
    // 冻结句柄自身仍拒绝其他写入者。目录根沿用普通排他重命名。
    let file = if root_identity.directory {
        open(root, true)?
    } else {
        OpenOptions::new()
            .access_mode(0x8000_0000 | READ_CONTROL.0 | DELETE.0)
            .share_mode((FILE_SHARE_READ | FILE_SHARE_WRITE).0)
...
pub(super) fn remove_matching(root: &Path, expected: &Snapshot) -> Result<(), Error> {
    ...
    for (relative, expected) in remaining.members.iter().rev() {
        let file = open(&member_path(root, relative), true)?;
