        ...
        for item in &serialized_items {
            match item {
                SerializedBlockListItem::Command { block } => {
                    // Zap 的 `to_serialized_blocklist_items` 复用持久化的 block id
                    // (CLI subagent 视图按 block id 归属,不能每次重新生成),
                    // 所以同一个 block 可能已经随 terminal 创建时的 serialized blocks
                    // 预加载进来了。没有这个守卫时,从历史列表把会话打开进一个已有
                    // terminal 会让同一条命令 block 出现两次。
