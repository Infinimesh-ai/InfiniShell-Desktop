...
maybe_define_setting!(EnableSshWarpification, group: WarpifySettings, {
    ...
    description: "Whether to enable InfiniShell features in SSH sessions.",
});
...
impl WarpifySettings {
    ...
    pub fn register(ctx: &mut AppContext) {
        ...
        // Zap:保留 tmux SSH wrapper 流程(上游 #12478 已删,我们不跟进),
        // 因此不执行上游把 `use_ssh_tmux_wrapper` 重置为 false 并弹一次性
        // 弃用提示的迁移 — 用户的 tmux 开关保持原值继续生效。
...
impl WarpifySettings {
    ...
    pub fn is_compatible_subshell_command(&self, command: &str, shell_family: ShellFamily) -> bool {
        ...
        // Zap:开启 tmux SSH wrapper 时,ssh 命令走 tmux warpify 流程,
        // 不在这里按普通 subshell 处理(上游删除 tmux 流程后去掉了该守卫,我们保留)。
