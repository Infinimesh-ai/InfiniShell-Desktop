...
impl Input {
    pub fn is_cloud_mode_input_v2_composing(&self, app: &AppContext) -> bool {
        FeatureFlag::CloudModeInputV2.is_enabled()
            ...
            && self.ambient_agent_view_model().is_some_and(|model| {
                ...
                // 上游这里还会排除 local→cloud handoff pane;Zap 的 AmbientAgentViewModel
                // 不持有 handoff 状态(该链路已剥离),该条件恒为真,故省略。
    ...
    pub(super) fn should_show_auth_secret_ftux(&self, app: &AppContext) -> bool {
        ...
        // Zap:上游还会查 `CloudAgentSettings.harness_auth_ftux_completed`,跳过
        // 已完成 FTUX 的 harness。该云端 agent 设置组未引入,无处记录"已完成",
        // 因此这一步跳过判断直接落到下面的"是否已选中 secret"上。
    ...
    fn render_cloud_mode_v2_history_menu(&self, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        // 上游为 V2 输入框单独持有一个 `cloud_mode_v2_history_menu_view`;我方未引入该
        // 视图类型,直接复用通用的 `inline_history_menu_view` —— V2 composing 分支与
        // 常规 agent 输入分支在 `render_agent_input` 里互斥,同一帧不会重复挂载。
