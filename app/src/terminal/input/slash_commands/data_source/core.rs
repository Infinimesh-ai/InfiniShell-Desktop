...
///
/// Zap:上游还在这里保存 `is_orchestration_enabled` / `is_cloud_handoff_enabled` /
/// `has_default_host` 三个门控,分别服务 `/orchestrate`、`/move-to-cloud` 和 `/host`。
/// 本地优先分支已删除编排模式、local→cloud 交接与自托管 host 选择,对应的静态命令也不在
/// 命令注册表里,因此这里只保留 CLI agent 富输入的门控。
    ...
    T: Entity<Event = UpdatedActiveCommands>,
{
    ...
    // Zap:上游还会监听 `ShouldForceDisableCloudHandoff` / `AIAutoDetectionEnabled` /
    // `PrivacySettingsChangedEvent::UpdateIsCloudConversationStorageEnabled` /
    // `UserWorkspacesEvent::{CodebaseContextEnablementChanged, TeamsChanged}`,它们分别驱动
    // 云端交接、云端会话存储与 codebase 索引相关的命令门控。这些功能在本地优先分支已下线
    // (对应的可用性位与门控也已剥离),重算不再依赖它们,故不订阅。
    ...
    // Recompute when the active conversation switches so commands gated on
    // `Availability::ACTIVE_CONVERSATION` update on navigation.
    //
    // Zap:上游还会订阅 `AgentConversationsModelEvent::{TasksUpdated, NewTasksReceived}`,
    // 目的仅是让 `/continue-locally` 在云端 task 拉取完成后出现。该命令属于 cloud→local
    // 交接链路,本地优先分支已删除,故不订阅。
    ctx.subscribe_to_model(
...
pub trait SlashCommandDataSource {
    ...
    fn base_availability(&self, ctx: &AppContext) -> Availability {
        ...
        // Zap:上游在这里补 `Availability::CODEBASE_CONTEXT`(由 `UserWorkspaces`
        // 的 codebase 上下文开关驱动)。codebase 向量索引整条链路在本地优先分支已下线,
        // 该可用性位也不存在,故不设置。
    ...
    fn command_passes_common_gates(
        ...
        // Zap:上游在这里还会按 `/orchestrate`、`/move-to-cloud`、`/host` 三个命令名做门控。
        // 这三个命令连同编排模式、local→cloud 交接、自托管 host 选择在本地优先分支都已删除,
        // 命令注册表里也没有对应常量,故整段剥离。
    ...
    fn ordered_zero_state_commands(&self, app: &AppContext) -> Vec<InlineItem> {
        ...
        // Zap:上游把 `/create-environment` 排在最前。云端环境创建在本地优先分支已删除,
...
impl InlineItem {
    ...
    pub(crate) fn from_saved_prompt(
        saved_prompt: &crate::workflows::WorkflowObject,
        app: &AppContext,
    ) -> Self {
        let appearance = Appearance::as_ref(app);
        Self {
