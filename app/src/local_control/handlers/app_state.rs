...
pub(crate) fn handle(
    ...
    match action {
        ...
        // Zap:agent 管理视图整片 UI 已删除(`WorkspaceAction::OpenAgentManagementView`
        // 不存在,`set_is_agent_management_view_open` 也是 no-op),
        // 这条 local-control 动作只能报不支持。
        ActionKind::SurfaceAgentManagementOpen => Err(ControlError::new(
            ErrorCode::UnsupportedAction,
            "surface.agent_management.open is not supported: the agent management view was removed",
        )),
