...
impl PaneGroup {
    ...
    pub(super) fn hydrate_task_backed_hidden_child_pane(
        ...
        let task_now = AgentConversationsModel::handle(ctx)
    ...
    /// Loads the transcript identified by `server_token` (Zap: resolved to a
    /// local conversation id, no cloud fetch), hydrates
    /// the placeholder via
    ...
    fn hydrate_remote_child_transcript_in_place(
        ...
        // Zap:上游这里调 `load_conversation_by_server_token`,直接向云端会话
        // 服务拉取转录。我方无该服务,改为用 server token 在本地索引里反查会话
        // id,再走本地 `load_conversation_data`;反查不到就直接落到
        // attach + 条件 tombstone,行为与云端 fetch 失败的那条分支一致。
        let local_conversation_id = BlocklistAIHistoryModel::as_ref(ctx)
            .find_conversation_id_by_server_token(&server_token);
        let Some(local_conversation_id) = local_conversation_id else {
            self.attach_ambient_session_and_maybe_tombstone(
                pane_id,
                child_id,
                task_id,
        ...
        let future =
            BlocklistAIHistoryModel::as_ref(ctx).load_conversation_data(local_conversation_id);
        ctx.spawn(future, move |group, conversation, ctx| {
            ...
            match conversation {
                Some(LoadedConversationData::Oz(cloud)) => {
