...
impl PaneGroup {
    ...
    /// own `BlocklistAIController` so child state doesn't cross the parent's
    /// single-stream state. Zap:上游的 viewer-side `Network` 已随云端
    /// shared session 删除,这里只提供本地 transcript 视图。
    pub(in crate::pane_group) fn ensure_shared_session_viewer_child_pane(
        &mut self,
        child_conversation_id: AIConversationId,
        // Zap:不再 join 远端 shared session,session id 仅保留在签名里以对齐
        // `Event::EnsureSharedSessionViewerChildPane` 的调用点。
        ...
        let view_size = Self::estimated_view_bounds(ctx).size();
        // Zap:上游在这里用 `shared_session::viewer::TerminalManager` 起一个云端
        // shared-session viewer,把子 agent 的实时 pty 流接进来。该 viewer 随
        // 云端 shared session 一并删除,这里退化为本地 transcript 视图:一个
        // loading terminal + 下面的会话恢复,渲染本地已有的子 agent 转录,
        // 不再 join 远端会话。
        let (new_terminal_view, terminal_manager) = Self::create_loading_terminal_manager_and_view(
            resources,
            view_size,
            ctx.window_id(),
            ctx,
        ...
        new_terminal_view.update(ctx, |terminal_view, ctx| {
            // Zap:`conversation_details_panel` 整个子系统已物理删除,
            // 因此不再需要抑制它的首次自动展开。
            terminal_view.restore_conversation_after_view_creation(
                RestoredAIConversation::new(child_conversation),
                true,
