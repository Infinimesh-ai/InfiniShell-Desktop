...
impl PaneGroup {
        ...
        let result = match leaf.contents {
            ...
            LeafContents::AmbientAgent(snapshot) => {
                ...
                let restore_kind = match &task_data {
                    ...
                    // metadata from BlocklistAIHistoryModel, which is loaded asynchronously.
                    // Defer to the pending-restoration handler so it can retry once that
                    // metadata arrives.
                    Some((task_id, _)) => {
            ...
            LeafContents::NetworkLog => {
                // 网络日志 pane 背后是内存 ring buffer,启动时为空,因此不持久化
                // (见 `LeafContents::is_persisted`),这里理论上不可达。
                Err(anyhow::anyhow!(
                    "Network log pane should not have been persisted, as it is not restorable"
                ))
            }
    ...
    fn create_ambient_agent_terminal(
        ...
        let (terminal_view, terminal_manager) =
            Self::create_ambient_agent_loading_terminal(resources, view_bounds_size, true, ctx);
