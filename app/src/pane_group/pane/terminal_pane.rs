...
// Zap:`orchestration_event_streamer` 依赖已删除的云端 server_api,未挂载到 blocklist 模块树。
...
// Zap:`retrieve_shared_session_link`(依赖已删除的 shared_session::manager 与
// join_link)随云端 session 分享一并移除。
...
fn kill_agent_conversation(
    ...
    // Zap:上游在此给被 Kill 的会话打墓碑,防止 streamer 的迟到事件把已移除的子 agent 复活。
    // orchestration event streamer 未挂载,本地编排没有迟到事件源,故省略。
...
fn handle_terminal_view_event(
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
            ...
            // Zap:OpenShareSessionModal / OpenShareSessionDeniedModal 随云端分享
            // 弹窗(share_modal)删除;仅保留本地子会话的连带停止分享。
            ...
            // ambient-agent UI 子系统物理删;`AnonymousUserSignup` 随登录/云端账号删除。
            #[cfg(feature = "local_fs")]
            ...
            // Zap:云端 agent 配额(capacity)提示弹窗随云端 agent 链路一并移除,
            // `terminal::view::Event` 与 `pane_group::Event` 都没有该 variant。
...
fn launch_remote_child(
    ...
    new_terminal_view.update(ctx, |terminal_view, ctx| {
        terminal_view.enter_agent_view(
            None,
            Some(conversation_id),
            // Zap 命名:上游的 `CloudAgent` 在我方叫 `AmbientAgent`。
