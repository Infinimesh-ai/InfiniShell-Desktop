...
pub(crate) fn terminal_view_agent_icon_variant(
    ...
    let inputs = TerminalIconInputs {
        ...
        // Zap 的 AmbientAgentViewModel 未保留上游的 `selected_third_party_cli_agent()`
        // 便捷方法,直接由选中的 harness 推导(语义等价)。
        selected_third_party_cli_agent: terminal_view
            .ambient_agent_view_model()
            .and_then(|model| CLIAgent::from_harness(model.as_ref(app).selected_harness())),
