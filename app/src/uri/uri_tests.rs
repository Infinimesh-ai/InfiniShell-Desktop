...
#[test]
fn test_app_web_link_rewrites_to_home() {
    // Zap:无云 agent,web 的 /app 链接与 /home 一样重写为本地 home intent。
    ...
    assert_eq!(
        intent.as_str(),
        format!("{}://home", ChannelState::url_scheme())
    );
}
...
fn test_settings_section_for_simple_subpage() {
    ...
    // Zap:云端账单 / OzCloudAPIKeys 页已删,对应子页不再映射。
