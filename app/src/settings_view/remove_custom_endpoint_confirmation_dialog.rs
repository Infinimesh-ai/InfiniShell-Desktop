...
impl RemoveCustomEndpointConfirmationDialog {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let confirm_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(
                crate::t!("settings-agent-providers-remove-endpoint-action"),
                DangerPrimaryTheme,
            )
            .on_click(|ctx| {
...
impl View for RemoveCustomEndpointConfirmationDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let description = crate::t!("settings-agent-providers-remove-endpoint-description");
        ...
        let dialog = Dialog::new(
            crate::t!("settings-agent-providers-remove-endpoint-title"),
            Some(description),
