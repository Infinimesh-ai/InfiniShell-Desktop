...
impl GitHubRepoModel {
    ...
    /// Feeds a `gh pr view` outcome through the real result-handling path so
    /// tests can observe PR-chip validation / suppression without running `gh`.
    pub(crate) fn apply_pr_fetch_result_for_test(
        ...
        match self {
            #[cfg(feature = "local_fs")]
            Self::Local(m) => m.update(ctx, |m, ctx| {
                m.apply_pr_fetch_result_for_test(branch, result, ctx)
            }),
            Self::Remote(_) => unreachable!("remote test models are not used"),
        }
    }
