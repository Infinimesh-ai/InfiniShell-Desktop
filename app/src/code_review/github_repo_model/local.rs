...
impl LocalGitHubRepoModel {
    ...
    /// Sets the tracked branch and feeds `result` through the real
    /// [`Self::handle_fetch_result`] path, so tests can exercise the
    /// validation / suppression semantics without spawning `gh`.
    pub(crate) fn apply_pr_fetch_result_for_test(
        &mut self,
        branch: &str,
        result: anyhow::Result<Option<PrInfo>>,
        ctx: &mut ModelContext<Self>,
        ...
        self.handle_fetch_result(result, branch.to_string(), ctx);
    }
}
