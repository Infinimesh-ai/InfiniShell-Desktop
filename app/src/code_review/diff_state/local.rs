...
impl LocalDiffStateModel {
    ...
    pub fn git_commit_chain(
        ...
        // Zap:PR 标题/正文的 AI 生成走的是云端 `server_api` 的 `AIClient`,该网关与
        // `ai::generate_code_review_content` 一并删除,提交链退化为纯 git 操作
        // (`gh pr create --fill` 兜底),`autogenerate_pr_content` 不再有作用。
        let _ = autogenerate_pr_content;
    ...
    pub fn create_pr(
        ...
        // Zap:同上,AI 生成 PR 内容的云端链路已删除,这里恒定走 `gh pr create --fill`。
        let _ = autogenerate_content;
        let path_future = Self::interactive_path_future(ctx);
        ctx.spawn(
            async move {
                let path_env = path_future.await;
    ...
    ///
    /// Zap:该能力依赖云端 `server_api` 的 `AIClient::generate_code_review_content`,
    /// 网关与请求模块均已删除。保留方法签名与事件契约(调用点/UI 不改),直接以错误
    /// 结果回落,由上层显示"不可用"。
    pub fn generate_commit_message(
        ...
        let _ = (include_unstaged, branch_name);
        let Some(_repo_path) = self.active_repository_path(ctx) else {
        ...
        ctx.emit(DiffStateModelEvent::CommitMessageGenerated(Err(
            "AI commit message generation is unavailable in InfiniShell".to_string(),
        )));
    }
    ...
    async fn get_file_diff(
        ...
        // 超大补丁最终也无法渲染，避免先把它完整解析成行和 hunk 结构。
