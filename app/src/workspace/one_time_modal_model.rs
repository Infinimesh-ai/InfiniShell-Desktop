...
use crate::auth::{AuthManager, AuthManagerEvent, AuthStateProvider};
use crate::channel::{Channel, ChannelState};
use crate::root_view::has_completed_local_onboarding;
// Zap(本地化,Phase 5):`PreferencesSyncer` 已物理删除,初始 modal 检查不再等待
// 云端偏好加载,`AuthComplete` 之后直接跑。
...
pub struct OneTimeModalModel {
    ...
    /// Whether the initial one-time modal checks have run. Event-driven re-checks
    /// wait for this so they never act before the startup queue has run.
...
impl OneTimeModalModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        // Workspace billing data arrives asynchronously; once it does we know whether
        // "no workspace" means a solo user and can re-evaluate deferred notices.
        ...
        ctx.subscribe_to_model(&AuthManager::handle(ctx), |me, _, event, ctx| {
            ...
            if is_existing_user {
                // Zap 无云端偏好同步,seen 标记只存在本地,直接跑一次检查即可。
