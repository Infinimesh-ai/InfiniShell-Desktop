...
pub(crate) fn initialize_app(app: &mut App) {
    ...
    // `AISettingsPageView::new` 在构造期取 `CLIAgentInstallModel` 句柄。
    app.add_singleton_model(crate::terminal::cli_agent::CLIAgentInstallModel::new);
    // Zap:通知中心单例。构造时订阅 BlocklistAIHistoryModel / CLIAgentSessionsModel,
    // 必须排在它们之后(与 `test_util/terminal.rs` 的注册顺序一致)。
    ...
    // OrchestrationEventService unconditionally, so the singleton must be
    // registered before bootstrap.
    app.add_singleton_model(
        crate::ai::blocklist::orchestration_events::OrchestrationEventService::new,
    );
    // 以下两个单例必须先注册:AgentConversationsModel / LLMPreferences 的 `new()`
    // 在构造期就会取它们。AgentProviderSecrets 是 Zap 的 BYOP 密钥存储,
    // 合并时该文件取了上游版,连带丢掉了这条注册。
    ...
    // `OneTimeModalModel::new` 在构造期就取 `AIRequestUsageModel` 句柄
    // (lib.rs 里前者位于 1937 行、后者位于 1529 行,顺序一致),
    // 所以这里必须先注册 usage model。
    ...
    App::test((), |mut app| async move {
        ...
        workspace.update(&mut app, |workspace, ctx| {
            ...
            assert!(
                ...
                "InfiniShell Drive should be open"
            );
            assert!(
                !workspace
                    .tips_completed
            ...
            assert!(
                !workspace.current_workspace_state.is_warp_drive_open,
                "InfiniShell Drive should be closed"
            );
            assert!(
                !workspace
                    .tips_completed
    ...
    App::test((), |mut app| async move {
        ...
        workspace.update(&mut app, |view, ctx| {
            assert!(
                ...
                "Expected InfiniShell Drive panel to be focused"
            );
        });
...
fn test_tools_panel_warp_drive_toggle_updates_available_views() {
    // Zap 无账号体系,`is_warp_drive_enabled` 直接跟随 `enable_warp_drive` 设置。
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let workspace = mock_workspace(&mut app);
