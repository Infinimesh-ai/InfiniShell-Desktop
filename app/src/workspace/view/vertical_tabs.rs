...
// 不再有 `TelemetryEvent` trait,这里无需再引入。
...
use crate::ui_components::icon_with_status::{
    IconWithStatusVariant, render_cli_agent_logo, render_icon_with_status,
};
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // InfiniShell Drive object types use their established index colors
...
fn has_unread_activity_for_terminal_view(terminal_view_id: EntityId, app: &AppContext) -> bool {
    ...
    // 本地通知中心的等价单例是 `notifications::model::NotificationsModel`。
...
///
/// Zap:上游在此叠加了云端环境名 / 环境准备状态(`cloud_environments` +
/// `AmbientAgentViewModel::selected_environment_id`),这条云端链路本地不存在,
/// 因此只保留本地工作目录。
