...
// Zap:遥测上报层已删除,这里仅保留事件类型壳供既有 `send_telemetry_from_ctx!` 调用点
// 继续类型检查;已删除 `impl TelemetryEvent` / `impl TelemetryEventDesc` /
// `register_telemetry_event!` 与 strum 的 `EnumDiscriminants` + `EnumIter` 派生。
#[derive(Debug)]
#[allow(dead_code)]
