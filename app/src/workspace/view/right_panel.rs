...
pub enum ReviewDestination {
    ...
    None,
    /// An InfiniShell agent terminal is available (input box visible, not executing).
    InfiniShell,
...
impl ReviewTerminalUnavailableReason {
    fn label(&self) -> &'static str {
        match self {
            ...
            Self::AIDisabled => "AI is disabled for InfiniShell review destinations",
...
impl RightPanelView {
    ...
    /// considered available (non-CLI InfiniShell terminals require AI to be on).
            ...
            .map(|tv| {
                tv.read(ctx, |t, ctx| {
                    t.active_cli_agent(ctx)
                        ...
                        .unwrap_or(ReviewDestination::InfiniShell)
                })
            })
