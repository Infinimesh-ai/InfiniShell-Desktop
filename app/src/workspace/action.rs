...
pub enum WorkspaceAction {
    ...
    ReopenClosedSession,
    // Zap:tab 菜单的云端分享动作(OpenShareSessionModal / StopSharing* /
    // CopySharedSessionLinkFromTab / OpenSharedSessionQrCode)随分享 UI 删除。
    AddWindow,
    AddWindowWithShell {
        shell: AvailableShell,
    },
    ...
    // 去中心化分支:`AttemptLoginGatedAIUpgrade`/`OpenCloudAgentSetupGuide`/
    // `OpenPromptSuggestionsUnavailableModal`(登录/套餐门控)已删除。
    ...
    // Zap:上游的 `OpenLocalToCloudHandoffPane` 打开 local→cloud 交接面板,
    // 其载荷类型 `ai::blocklist::handoff::PendingCloudLaunch` 与
    // `ai::ambient_agents::telemetry::HandoffEntryPoint` 随云端交接链路一起下线,
    // 本地优先分支不再有"交接到云端"这个动作,整个变体已删除。
    ...
    // Zap:`OpenAgentManagementView` 随 agent 管理 UI 删除;Build Plan 迁移弹窗
    // (套餐相关)不引入。
    ...
    OpenNetworkLogPane,
    // Zap:`OpenNewWindowForTeam` / `ShowTeamSwitcherMenu`(team/账号功能)不引入。
}
