...
// Zap:上游这里用的是 `ServerNotebook` + `ServerMetadata`/`ServerPermissions`(云端同步对象),
// 我方对应的本地对象体系是 `NotebookObject` + `StoredObjectMetadata`/`StoredObjectPermissions`。
// 测试覆盖的行为(zero-state / 过滤态下按 revision 时间给出新近度加分)在 Zap 中依然存在,
// 因此保留测试,只把种子数据换成本地对象。
