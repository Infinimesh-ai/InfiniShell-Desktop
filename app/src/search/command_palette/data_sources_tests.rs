...
fn test_full_text_drive_data_source_indexes_an_object_that_moves_into_the_window() {
    ...
    App::test((), |mut app| async move {
        ...
        // The workflow is moved into this window's team drive.
        // Zap:服务端下推的权限更新路径(`update_object_permissions` + `UpdateSource::Server`)
        // 已随云端同步剥离,改走本地移动路径 —— 对 data source 而言同样是
        // "对象换了 owner space",索引重建行为与覆盖意图一致。
        ObjectStoreModel::handle(&app).update(&mut app, |model, ctx| {
    ...
    App::test((), |mut app| async move {
        ...
        ObjectStoreModel::handle(&app).update(&mut app, |model, ctx| {
            model.seed_workflow(
                WorkflowObject::new(
                    SyncId::ServerId(WorkflowId::from(1).into()),
                    WorkflowObjectModel::new(Workflow::new("preexisting shared", "echo shared")),
                    mock_server_metadata(),
                    mock_shared_server_permissions(shared_with),
                ),
                ctx,
