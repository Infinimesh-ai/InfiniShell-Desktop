...
fn render_state_label(state: &TransferState, appearance: &Appearance) -> Box<dyn Element> {
    ...
    let (label, color) = match state {
        TransferState::Pending => (
            crate::t!("sftp-transfer-pending"),
            theme.sub_text_color(theme.background()),
        ),
        TransferState::InProgress => (crate::t!("sftp-transfer-in-progress"), theme.accent()),
        TransferState::Completed => (
            crate::t!("sftp-transfer-completed"),
            theme.ui_green_color().into(),
        ),
        TransferState::Failed(_) => (
            crate::t!("sftp-transfer-failed"),
            theme.ui_error_color().into(),
        ),
        TransferState::Cancelled => (
            crate::t!("sftp-transfer-cancelled"),
            theme.sub_text_color(theme.background()),
        ),
    };
