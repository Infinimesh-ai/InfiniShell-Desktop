...
// Zap:本地 auth facade 是扁平模块,沿用 `auth::{...}` 直接导出。
...
impl LaunchMode {
    ...
    fn execution_mode(&self) -> ExecutionMode {
        match self {
            ...
            // RemoteServerDaemon gets its own execution mode so headless daemon
            // behaviour (no autoupdate, no GUI surfaces) is distinguishable.
...
pub fn run() -> Result<()> {
    ...
    {
        // SAFETY: edition 2024 起 `set_var` 是 unsafe。此处在 `main` 早期、
        // 任何工作线程 spawn 之前执行(仅 `platform::init()` 之前的单线程阶段),
        // 不存在其它线程并发读写环境变量,满足安全前提。
        unsafe {
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    // local-only config behavior for this launch mode.
...
impl StartupUserAuthentication {
    fn start(self, ctx: &mut AppContext) {
        // Zap:本地 auth facade 没有云端 API key 校验通道,两种入口都只做一次
        // 本地身份刷新(`refresh_user` 在本地实现里是 no-op)。
        match self {
            Self::RefreshUser | Self::ApiKey(_) => {
                AuthManager::handle(ctx)
                    .update(ctx, |auth_manager, ctx| auth_manager.refresh_user(ctx));
            }
        }
    }
}

/// Whether startup user authentication should proceed without blocking.
///
/// Upstream gates this on the staging IAP handshake; Zap has no IAP, so this
/// only records that the TUI front-end must never stall its startup on auth.
...
pub(crate) fn initialize_app(
    ...
    // Zap:本地 auth facade 没有独立的 credential-validation 路径,api key 直接
    // 交给 `AuthState::initialize` 装载本地身份快照。
    let (auth_state, pending_api_key) = match launch_mode.auth_initialization() {
        AuthInitialization::Persisted => (AuthState::initialize(ctx, None), None),
        AuthInitialization::PendingApiKey(api_key) => (
            AuthState::initialize(ctx, Some(api_key.clone())),
            Some(api_key),
        ),
    };
    ...
    // BYOK 密钥变化后刷新可用模型/额度视图。
    // Zap:上游同时订阅 UserWorkspaces 的云端 overage / addon 事件,本地化后不适用。
    ctx.subscribe_to_model(
        &::ai::api_keys::ApiKeyManager::handle(ctx),
        |_, event, ctx| {
            let ::ai::api_keys::ApiKeyManagerEvent::KeysUpdated = event;
            // Zap:上游这里调 `request_availability_refresh()`(拉云端额度可用性),
            // 该接口随云端计量一起下线。本地只保留 `refresh_request_usage_async()`
            // 这个 no-op 钩子,后续如需本地计量可在此接入。
    ...
    // 清理上一次 auto-update 遗留的旧可执行文件,无论本次是否由 auto-update 拉起。
    //
    // macOS 上这会删掉已安装 bundle 内的 `Contents/MacOS/old`,所以和上游一样加
    // `can_autoupdate` 守卫:永不自更新的执行模式不得改写该 bundle(见 APP-2946)。
    //
    // Zap:清理本身丢到 background_executor —— 跨平台路径里只有 macOS 实际要做事
    // (Linux/Windows 是 noop),且只是 fs::remove_dir_all,与主线程后续逻辑零依赖。
    ...
    // server API token rotation 事件一同物理删;云端 codebase indexing 也已下线,
    // 因此不注册 `RemoteCodebaseIndexModel`。
        ...
        .collect();
    {
        ...
        // Zap:NLD prompt 历史快照未纳入本地持久化(`PersistedData` 无 `nld_prompts`),
        // 因此这里始终传空,`NldPromptHistoryMatch` 只影响运行期采集。
        ctx.add_singleton_model(move |_| {
            BlocklistAIHistoryModel::new(ai_queries, Vec::new(), conversations)
        });
    }
    ...
    // Zap(本地化):上游在这里注册 `IapManager`(staging 环境的 Google IAP token
    // 刷新)以及依赖它的启动认证门控。Zap 不连 warp-server,`warp_server_client`
    // 已从 workspace 移除,整块 IAP 机制随之物理删除。
    //
    // 启动认证退化为一次本地身份刷新:CLI 走自己的分发路径,其它前端在已登录
    // (或带 api key)时触发。
    ...
    // Zap:云端 codebase index(`CodebaseIndexManager`)整个子系统已随
    // `ai::index::full_source_code_embedding` 一起物理删除,这里只注册项目规则模型。
    ctx.add_singleton_model(|ctx| {
