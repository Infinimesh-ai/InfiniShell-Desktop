...
impl NotificationMailboxView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let model_handle = NotificationsModel::handle(ctx);
        ctx.subscribe_to_model(&model_handle, |me, _handle, event, ctx| match event {
            NotificationsEvent::NotificationAdded { .. }
            | NotificationsEvent::NotificationUpdated
            | NotificationsEvent::AllNotificationsMarkedRead => {
        ...
        let close_button = ctx.add_typed_action_view(|_| {
            ActionButton::new("", NakedTheme)
                ...
                .with_tooltip(crate::t!("common-close"))
        ...
        let mark_all_read_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(crate::t!("ai-notifications-mark-all-as-read"), NakedTheme)
    ...
    fn render_item_at_index(&self, index: usize, app: &AppContext) -> Box<dyn Element> {
        let notifications = NotificationsModel::as_ref(app).notifications();
        let Some(item) = self
            .filtered_ids
            .get(index)
