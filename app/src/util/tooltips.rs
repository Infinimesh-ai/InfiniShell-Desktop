...
/// Returns whether "Open in InfiniShell" should be offered for the given file path.
///
/// This checks:
...
/// - Whether the file renders in Zap's notebook viewer, which is reached via a
...
pub fn should_show_open_in_warp_link(path: &Path, app: &AppContext) -> bool {
    ...
    if matches!(*settings.open_file_editor, EditorChoice::InfiniShell) {
        return false;
    }
