...
pub fn open_file_path_with_line_and_col(
    ...
    if full_path.is_file() {
        ...
        // InfiniShell channel (e.g. Stable handling files while Preview is
        // running). When the resolved default is a different InfiniShell, open
        // with the running channel's bundle directly.
        ...
        if let Some(bundle_id) = bundle_id.as_deref() {
            ...
            if bundle_id != current
                && is_infinishell_bundle(bundle_id)
...
fn is_infinishell_bundle(bundle_id: &str) -> bool {
    AppId::parse(bundle_id)
        .map(|id| id.qualifier() == "dev" && matches!(id.organization(), "warp" | "infinishell"))
        .unwrap_or(false)
}
