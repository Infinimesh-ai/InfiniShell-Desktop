...
/// Registers the singleton models every GitHub-PR-chip test needs.
#[cfg(feature = "local_fs")]
fn init_app_for_github_pr_chip_tests(app: &mut App) {
    app.add_singleton_model(|_| {
        Prompt::mock_with(
    ...
    app.add_singleton_model(AuthManager::new_for_test);
    app.add_singleton_model(|_| crate::settings::manager::SettingsManager::default());
    crate::settings::InputSettings::register(app);
    app.update(crate::settings::AISettings::register_and_subscribe_to_events);
    app.add_singleton_model(crate::workspaces::user_workspaces::UserWorkspaces::default_mock);
    #[cfg(windows)]
    app.add_singleton_model(SystemInfo::new);
...
/// Builds an inert per-repo `GitHubRepoModel` (never spawns `gh`) tracking `branch`.
#[cfg(feature = "local_fs")]
fn github_repo_model_for_test(
    app: &mut App,
    branch: &str,
    ...
    let repo_handle = watcher_handle.update(app, |watcher, ctx| {
        watcher
            .add_directory(
...
#[cfg(feature = "local_fs")]
fn github_pr_info_for_test(number: u64) -> PrInfo {
    PrInfo {
        number,
...
#[cfg(feature = "local_fs")]
fn github_pr_chip_value(current_prompt: &CurrentPrompt) -> Option<String> {
    current_prompt
...
/// Deterministic `gh`-not-installed failure: the PR chip must be dropped from the
/// default prompt, and re-checked once `gh` shows up on `PATH` again.
///
/// The PR chip no longer shells out itself — `gh pr view` now runs once per repo in
/// `LocalGitHubRepoModel`, so "is `gh` installed" is answered by the model's fetch
/// error rather than by a `RequiresExecutable` chip dependency.
#[cfg(feature = "local_fs")]
#[test]
fn test_github_pr_chip_is_disabled_when_github_cli_is_missing() {
    ...
    App::test((), |mut app| async move {
        ...
        init_app_for_github_pr_chip_tests(&mut app);

        // `gh` is on `PATH` for this session, so the suppression must be lifted
        // again once the session is (re)established.
        ...
        let (_temp_dir, github_repo_model) = github_repo_model_for_test(&mut app, "feature-a");
        let sessions_for_prompt = sessions.clone();
        let current_prompt =
            app.add_model(move |ctx| CurrentPrompt::new(sessions_for_prompt.clone(), ctx));
        ...
        current_prompt.update(&mut app, |current_prompt, ctx| {
            ...
            current_prompt.set_github_repo_model(Some(github_repo_model.downgrade()), ctx);
            current_prompt.update_states_with_new_context(ctx);
        });

        // The chip itself must never shell out: its value comes from the repo model.
        assert!(executor.commands.lock().is_empty());

        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "feature-a",
                Err(anyhow::anyhow!(
        ...
        app.read(|ctx| {
            assert_eq!(
                *SessionSettings::as_ref(ctx).github_pr_chip_default_validation,
                ...
                "a missing `gh` must drop the PR chip from the default prompt"
            );
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(github_pr_chip_value(current_prompt), None);
        });

        // Suppression is persisted, so it has to be re-evaluated on session changes:
        // `gh` is on this session's `PATH`, so the chip gets another chance.
        current_prompt.update(&mut app, |current_prompt, ctx| {
            current_prompt.update_states_with_new_context_and_session(ctx);
        });
        app.read(|ctx| {
            assert_eq!(
                *SessionSettings::as_ref(ctx).github_pr_chip_default_validation,
                GithubPrPromptChipDefaultValidation::Unvalidated,
                "an installed `gh` must un-suppress the PR chip default"
            );
        });
    });
}

/// An authoritative "this branch has no PR" result is a success, not a failure:
/// it must validate the chip rather than suppress it.
#[cfg(feature = "local_fs")]
#[test]
fn test_github_pr_chip_empty_success_does_not_set_failure_suppression() {
    ...
    App::test((), |mut app| async move {
        ...
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test("feature-a", Ok(None), ctx);
        });

        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt),
                None,
                "chip value should be None after an empty result"
            );
        });
        app.read(|ctx| {
            assert_eq!(
                *SessionSettings::as_ref(ctx).github_pr_chip_default_validation,
                ...
                "an empty result proves `gh` works, so it must validate, not suppress"
            );
        });
    });
}

/// Returning to a branch without a PR must clear the previously displayed URL
/// instead of leaving a stale link on screen.
#[cfg(feature = "local_fs")]
#[test]
fn test_github_pr_chip_revisiting_empty_result_directory_reruns_and_clears_previous_value() {
    ...
    App::test((), |mut app| async move {
        init_app_for_github_pr_chip_tests(&mut app);
        let (_temp_dir, github_repo_model) = github_repo_model_for_test(&mut app, "no-pr");
        ...
        // Branch without a PR.
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test("no-pr", Ok(None), ctx);
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(github_pr_chip_value(current_prompt), None);
        });
        ...
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "has-pr",
                Ok(Some(github_pr_info_for_test(456))),
                ctx,
            );
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt).as_deref(),
                Some("https://github.com/warp/warp/pull/456")
            );
        });

        // Back to the branch without a PR: the stale URL must be dropped.
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test("no-pr", Ok(None), ctx);
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt),
                None,
                "revisiting a branch with no PR must clear the previous value"
            );
        });
        app.read(|ctx| {
            assert_eq!(
...
/// A deterministic `gh` auth failure suppresses the chip and drops any cached PR,
/// and a later success brings both back.
#[cfg(feature = "local_fs")]
#[test]
fn test_github_pr_chip_revisiting_failed_directory_uses_failure_suppression() {
    ...
    App::test((), |mut app| async move {
        ...
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "feature-a",
                Err(anyhow::anyhow!("gh: authentication required")),
                ctx,
            );
        });
        ...
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt),
                None,
                "a deterministic auth failure must drop the cached PR"
            );
        });

        // Once `gh` is authenticated again, the chip recovers.
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "feature-a",
                Ok(Some(github_pr_info_for_test(456))),
...
/// A transient (network) failure must not be treated as a deterministic one: the
/// cached PR stays on screen, the chip is not suppressed, and the next attempt
/// still runs.
#[cfg(feature = "local_fs")]
#[test]
    ...
    App::test((), |mut app| async move {
        ...
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "feature-a",
                Err(anyhow::anyhow!(
                    "Post \"https://api.github.com/graphql\": dial tcp: \
        ...
        app.read(|ctx| {
            assert_eq!(
                *SessionSettings::as_ref(ctx).github_pr_chip_default_validation,
                GithubPrPromptChipDefaultValidation::Validated,
                "a transient failure must not suppress the chip"
            );
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt).as_deref(),
                Some("https://github.com/warp/warp/pull/456"),
                "a transient failure must not flash the PR chip away"
            );
        });

        // The retry succeeds and the value is refreshed.
        github_repo_model.update(&mut app, |model, ctx| {
            model.apply_pr_fetch_result_for_test(
                "feature-a",
                Ok(Some(github_pr_info_for_test(789))),
                ctx,
            );
        });
        current_prompt.read(&app, |current_prompt, _| {
            assert_eq!(
                github_pr_chip_value(current_prompt).as_deref(),
                Some("https://github.com/warp/warp/pull/789")
            );
        });
    });
}
