...
impl CurrentPrompt {
    ...
    /// On session changes (including app startup), re-check whether a previously suppressed PR
    /// chip should get another chance.
    ///
    /// Suppression is sticky across restarts (it is a persisted setting), so without this the
    /// chip would stay hidden forever after a single `gh`-missing / `gh`-not-authenticated
    /// failure. If `gh` is now on `PATH`, resetting to `Unvalidated` lets
    /// [`crate::code_review::github_repo_model::LocalGitHubRepoModel`] re-validate or
    /// re-suppress on its next fetch.
