...
原生 Grok 插件，版本 `0.1.0`，受测 Grok `1.0.30`。无需远端 marketplace。插件仅把已知生命周期事件转换为 InfiniShell OSC 777 v1 通知，不改变权限策略。
...
- `StopFailure` 保持失败状态；`SessionEnd` 和 `Stop(reason=shutdown)` 不能变成成功。只有显式 `end_turn`、有最终文本、没有继续 hook 标志且当前回合未失败时才发送完成；未实测的普通 Stop 降级为通知。
