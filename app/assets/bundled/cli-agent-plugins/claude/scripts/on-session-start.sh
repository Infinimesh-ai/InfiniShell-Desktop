...
# Claude 提供的插件根目录已经过 argv 路径验证；Windows Git Bash 对含 shell
# 元字符的 BASH_SOURCE 反推路径可能失真，因此只在未提供根目录时回退。
PLUGIN_MANIFEST="${CLAUDE_PLUGIN_ROOT:-$SCRIPT_DIR/..}/.claude-plugin/plugin.json"
PLUGIN_VERSION=$(jq -r '.version // "unknown"' "$PLUGIN_MANIFEST" 2>/dev/null)
