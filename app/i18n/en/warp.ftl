...
cli-agent-grok-managed-unverified = This Grok operation or permission policy is not supported by the verified managed interface. Use the inherited CLI settings for root tasks; SDK tools, child tasks, images, and skills remain unverified.
...
cli-task-manager-grok-unavailable = Grok Build 1.0.30 supports root text tasks with inherited CLI settings, approvals, cancellation, and history continuation. Added instructions wait for a later turn. SDK tools, child tasks, images, and skills remain unavailable.
...
cli-agent-grok-input-skills-unverified = Managed skill input has not been verified for Grok Build. Keep the selected skill in this draft.
cli-task-manager-grok-queued = While Grok is running, added instructions wait for a later turn. They do not change the current execution. Receipt is confirmed only when Grok accepts the instruction. Cancelling the current turn may let a waiting instruction start afterward.
