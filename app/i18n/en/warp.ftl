...
settings-agent-providers-name-placeholder = Custom provider name (e.g. My API gateway)
...
settings-agent-providers-models-empty-hint = No models configured yet. Click [+ Add model] to add one manually, or [Sync API models] to load the provider's current list.
...
settings-agent-providers-model-auto-value = Auto: { $value }
...
settings-agent-providers-model-state-on = On
settings-agent-providers-model-state-off = Off
settings-agent-providers-model-state-auto = Auto ({ $value })
settings-agent-providers-model-state-label = { $label }: { $state }
settings-agent-providers-models-dev-mapping = Advanced models.dev mapping
settings-agent-providers-models-dev-provider-placeholder = Provider ID (Auto)
settings-agent-providers-models-dev-model-placeholder = Catalog model ID (Auto: use API model ID)
settings-agent-providers-models-dev-mapping-hint = Leave both fields empty for automatic matching. Set them only when a custom gateway should inherit metadata from a specific models.dev entry.
settings-agent-providers-models-dev-match-explicit = explicit mapping
settings-agent-providers-models-dev-match-provider = provider + model match
settings-agent-providers-models-dev-match-unique = globally unique model ID
settings-agent-providers-models-dev-snapshot-current = current snapshot
settings-agent-providers-models-dev-snapshot-stale = cached snapshot (refresh recommended)
settings-agent-providers-models-dev-snapshot-unmatched = not in the latest catalog; previous snapshot retained
settings-agent-providers-models-dev-match-status = models.dev: { $provider } / { $model } · { $confidence } · { $freshness }
settings-agent-providers-models-dev-match-none = models.dev: No unambiguous catalog match. Conservative defaults are in use.
settings-agent-providers-models-dev-overrides-hint = Overrides take priority. Values imported from older settings may also count as overrides; use Reset manual overrides to use catalog values.
settings-agent-providers-reset-model-overrides = Reset manual overrides ({ $count })
...
settings-agent-providers-fetch-from-api = Sync API models
settings-agent-providers-fetch-hint = Removes missing API-discovered models only; keeps manually added and older configured models.
settings-agent-providers-api-empty-retained = The API returned no models, so the current list was kept. Check the endpoint and try again.
...
settings-agent-providers-models-dev-updated = Refreshed catalog metadata for { $count } models; preserved { $overrides } manual overrides.
settings-agent-providers-models-dev-partial = Refreshed { $matched } catalog matches and preserved { $overrides } overrides; { $retained } previous snapshots were not found in the latest catalog.
settings-agent-providers-models-dev-up-to-date = Model metadata is already up to date.
settings-agent-providers-models-dev-no-match = No configured models were found in models.dev.
settings-agent-providers-models-dev-no-match-retained = No unambiguous catalog match was found; retained { $count } previous metadata snapshots.
settings-agent-providers-models-dev-load-failed = Unable to load models.dev. Check your network connection and try again.
settings-agent-providers-model-id-reset = Changed { $count } model IDs. Old model metadata, mappings, and unchanged manual overrides were reset before rematching.
...
ai-error-context-too-large = The request context is too large. Compact the conversation or reduce attached content, then try again.
...
ai-export-requested-command = Requested command
ai-export-command-request-cancelled = Command request cancelled
ai-export-command-result-unavailable = Command result unavailable
agent-stop-task = Stop task
agent-stop-task-and-command = Stop task and interrupt its command
ai-error-conversation-update-failed = Failed to update this task. Please try again or start a new task.
...
cli-agent-grok-managed-unverified = This Grok operation or policy is unavailable. Child tasks require a matching fixed read or file policy. PNG images require verified Grok Build 1.0.41, grok-4.7, Inherit, and no selected skill. Skills require Inherit and a matching skill in the current session.
...
cli-agent-plugin-grok-restart-note = After installing the requirements, retry the notification setup. Run /plugins in Grok and press r on the Plugins tab to load the installed or enabled plugin.
cli-agent-plugin-grok-installed = InfiniShell plugin installed. Run /plugins in Grok and press r on the Plugins tab to reload.
cli-agent-plugin-grok-updated = InfiniShell plugin updated. Run /plugins in Grok and press r on the Plugins tab to reload.
...
cli-task-manager-queue-next-turn = Queue instruction
cli-task-manager-claude-queued = Claude has an instruction waiting to start. It may join the current execution between tool calls or start a later turn. Confirmed receipt does not mean completion. Cancel turn interrupts the current execution; an instruction still waiting may run later.
cli-agent-claude-queue-full = A Claude instruction is already waiting to start. Keep this draft until that input starts.
cli-agent-grok-queue-full = The Grok input queue is full. Keep this draft until a queued instruction starts.
cli-agent-claude-queue-uncertain = The queued Claude instruction's execution could not be confirmed. Review the native session before continuing; this input will not be resent automatically.
...
cli-agent-plugin-codex-windows-dependencies = Windows notification setup requires system Windows PowerShell 5.1 and cmd, Git Bash usr/bin, and jq with --binary support on PATH. Custom cmd AutoRun and BASH_ENV/ENV startup scripts are not supported.
cli-agent-plugin-codex-patch-manual-note = Native installation alone does not apply InfiniShell’s notification patch. Windows automatic setup supports the verified native Codex 0.147.0 executable on x64 and checks its notification dependencies before installation. For SSH or containers, transfer the bundled patch and run its verification script on that host.
...
cli-agent-grok-image-format-unverified = Managed Grok tasks currently support PNG images with verified Grok Build 1.0.41 and grok-4.7. Your draft is unchanged.
cli-agent-claude-image-skill-unverified = Images and skills have not been verified together for this version. Submit them in separate turns.
cli-agent-input-text-too-large = Text input must be 1 MiB or smaller.
cli-agent-runtime-data-too-large = CLI data exceeds the size limit.
cli-agent-claude-image-replay-invalid = The CLI returned image input that does not match this turn.
...
cli-agent-task-skills-require-managed = Skill delivery requires a managed Codex, Claude, or Grok task.
...
cli-agent-claude-file-policy-unverified = Cannot verify the Claude file task policy. Check the CLI permission settings before starting or continuing a task.
cli-task-manager-show-permission-details = Show permission details
cli-task-manager-hide-permission-details = Hide permission details
...
cli-task-manager-grok-queued = Added Grok instructions wait for a later turn and are confirmed only when Grok accepts them. With local task tools enabled, cancelling a turn closes the connection and leaves queued instructions unsent; continue the task explicitly. Otherwise, a queued instruction may start after cancellation.

cli-task-manager-permission-grok-read = Grok: fixed read tools and per-call approval
cli-task-manager-permission-grok-files = Grok: fixed file tools and per-call approval
cli-task-manager-permission-grok-p0 = Grok: verified P0 native reads
cli-task-manager-permission-grok-read-help = Use an app-owned Grok configuration with fixed read tools and approval for each call. Hooks, skills, and shell commands are disabled. This is not a filesystem sandbox; system administrator policies still apply.
...
settings-cli-updates-channel-stable = Stable
settings-cli-updates-channel-alpha = Alpha (preview)
settings-cli-updates-channel-codex = Latest is the regular release. Alpha contains preview changes. Following the installation preserves its channel.
settings-cli-updates-channel-claude = Latest is the default; Stable receives releases later. Updates apply your selected channel to Claude’s settings. Following the installation preserves its channel.
settings-cli-updates-channel-grok = Stable is the regular release; Alpha is a preview. Updates apply your selected channel to Grok’s settings. Following the installation preserves its channel.
settings-cli-updates-effective-channel = Detected target channel: { $channel }
settings-cli-updates-not-checked = Updates have not been checked yet.
settings-cli-updates-version-unknown = Unknown
settings-cli-updates-versions = Installed: { $installed } · Target: { $latest } · Source: { $source }
settings-cli-updates-check = Check for updates
settings-cli-updates-update = Update when idle
settings-cli-updates-install-guide = Installation guide
settings-cli-updates-source-native = Official installer
settings-cli-updates-source-unknown = Unrecognized
settings-cli-updates-checking = Checking the selected release channel…
settings-cli-updates-current = This installation matches the selected channel.
...
settings-cli-updates-probe-failed = The installed CLI could not report its version. Check the installation and try again.
settings-cli-updates-permission = The installation cannot be updated with current permissions. Use its original installer.
settings-cli-updates-command-failed = The installer reported a failure. Check the installation before retrying.
settings-cli-updates-timeout = The update timed out. Check the installation before retrying.
settings-cli-updates-version-mismatch = The installed version does not match the selected target. Check again before retrying.
settings-cli-updates-channel-mismatch = This installation source cannot switch to the selected channel automatically. Use the official installer to change channels.
settings-cli-updates-recovery = A previous update needs recovery. Check for updates to inspect the installation before retrying.
settings-cli-updates-persistence = The update record could not be saved. Check local storage and try again.
