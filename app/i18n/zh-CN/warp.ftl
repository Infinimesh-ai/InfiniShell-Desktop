...
settings-agent-providers-description = 配置支持多种协议的自定义智能体提供商，包括兼容 OpenAI 的服务（DeepSeek、智谱 GLM、Moonshot、通义千问 DashScope、SiliconFlow、OpenRouter 等）、Anthropic、Gemini 和本地 Ollama。你可以手动添加模型，也可以同步提供商 API 返回的模型。API 刷新只移除失踪的 API 发现模型；手动添加及旧配置模型仍会保留。新模型会自动从 models.dev 补全上限和能力元数据，该目录绝不会向列表追加模型。提供商元数据存储在本地 settings.toml，API 密钥则安全存储在系统密钥库中。
...
settings-agent-providers-name-placeholder = 自定义提供商名称（例如：我的 API 网关）
...
settings-agent-providers-models-empty-hint = 尚未配置模型。点击“+ 添加模型”可手动添加，点击“同步 API 模型”可加载提供商的当前列表。
...
settings-agent-providers-model-auto-value = 自动：{ $value }
...
settings-agent-providers-model-state-on = 开启
settings-agent-providers-model-state-off = 关闭
settings-agent-providers-model-state-auto = 自动（{ $value }）
settings-agent-providers-model-state-label = { $label }：{ $state }
settings-agent-providers-models-dev-mapping = 高级 models.dev 映射
settings-agent-providers-models-dev-provider-placeholder = 供应商 ID（自动）
settings-agent-providers-models-dev-model-placeholder = 目录模型 ID（自动沿用 API 模型 ID）
settings-agent-providers-models-dev-mapping-hint = 两项留空即自动匹配。仅当自定义网关需要继承 models.dev 中某个指定条目的元数据时填写。
settings-agent-providers-models-dev-match-explicit = 显式映射
settings-agent-providers-models-dev-match-provider = 供应商与模型匹配
settings-agent-providers-models-dev-match-unique = 全目录唯一模型 ID
settings-agent-providers-models-dev-snapshot-current = 当前快照
settings-agent-providers-models-dev-snapshot-stale = 已缓存快照（建议刷新）
settings-agent-providers-models-dev-snapshot-unmatched = 最新目录未匹配，已保留旧快照
settings-agent-providers-models-dev-match-status = models.dev：{ $provider } / { $model } · { $confidence } · { $freshness }
settings-agent-providers-models-dev-match-none = models.dev：没有无歧义的目录匹配，当前使用保守默认值。
settings-agent-providers-models-dev-overrides-hint = 覆盖值优先。旧配置导入的值也可能被视为覆盖；如需使用目录值，可点击“重置手动覆盖”。
settings-agent-providers-reset-model-overrides = 重置手动覆盖（{ $count } 项）
...
settings-agent-providers-fetch-from-api = 同步 API 模型
settings-agent-providers-fetch-hint = 仅移除失踪的 API 发现模型；保留手动添加及旧配置模型。
settings-agent-providers-api-empty-retained = API 未返回模型，已保留现有列表；请确认端点后重试。
...
settings-agent-providers-models-dev-updated = 已刷新 { $count } 个模型的目录元数据，并保留 { $overrides } 项手动覆盖。
settings-agent-providers-models-dev-partial = 已刷新 { $matched } 个目录匹配并保留 { $overrides } 项覆盖；另有 { $retained } 个旧快照未在最新目录中匹配。
settings-agent-providers-models-dev-up-to-date = 模型元数据已是最新。
settings-agent-providers-models-dev-no-match = models.dev 中未找到已配置的模型。
settings-agent-providers-models-dev-no-match-retained = 未找到无歧义的目录匹配；已保留 { $count } 个既有元数据快照。
settings-agent-providers-models-dev-load-failed = 无法加载 models.dev，请检查网络后重试。
settings-agent-providers-model-id-reset = 已更改 { $count } 个模型 ID；重新匹配前已重置旧模型元数据、映射和未改动的手动覆盖。
...
ai-error-context-too-large = 请求上下文过大。请压缩会话或减少附加内容后重试。
...
ai-export-requested-command = 请求执行的命令
ai-export-command-request-cancelled = 命令请求已取消
ai-export-command-result-unavailable = 命令结果不可用
agent-stop-task = 停止任务
agent-stop-task-and-command = 停止任务并中断正在运行的命令
ai-error-conversation-update-failed = 更新任务失败。请重试或新建任务。
...
cli-agent-grok-managed-unverified = Grok 不支持这项操作或策略。子任务需要与父任务匹配的固定只读或文件工具策略。PNG 图片需要已验证的 Grok Build 1.0.41、grok-4.7、“继承”策略，且不能同时选择技能。技能需要使用“继承”策略，并匹配当前会话中的技能。
...
cli-agent-plugin-grok-restart-note = 安装所需运行时后，请重试通知设置。在 Grok 中运行 /plugins，然后在 Plugins 页按 r 加载已安装或启用的插件。
cli-agent-plugin-grok-installed = InfiniShell 插件已安装。在 Grok 中运行 /plugins，然后在 Plugins 页按 r 重新加载。
cli-agent-plugin-grok-updated = InfiniShell 插件已更新。在 Grok 中运行 /plugins，然后在 Plugins 页按 r 重新加载。
...
cli-task-manager-queue-next-turn = 排队追加指令
cli-task-manager-claude-queued = Claude 有一条指令尚未开始。它可能在工具调用之间并入当前执行，也可能开始后续回合。确认收到不表示完成。取消当前回合会中断当前执行；仍在等待的指令可能随后执行。
cli-agent-claude-queue-full = Claude 已有一条指令等待开始。请保留草稿，等该输入开始后再发送。
cli-agent-grok-queue-full = Grok 输入队列已满。请保留草稿，等排队中的指令开始后再发送。
cli-agent-claude-queue-uncertain = 无法确认排队的 Claude 指令是否执行。继续前请检查原生会话；应用不会自动重发这条输入。
...
cli-agent-plugin-codex-windows-dependencies = Windows 通知安装需要系统 Windows PowerShell 5.1 和 cmd，以及 PATH 中的 Git Bash usr/bin 和支持 --binary 的 jq。不支持自定义 cmd AutoRun 或 BASH_ENV/ENV 启动脚本。
cli-agent-plugin-codex-patch-manual-note = 仅执行原生安装不会应用 InfiniShell 通知修补。Windows 自动安装仅支持已验证的 x64 原生 Codex 0.147.0 可执行文件，并会在安装前检查通知依赖。SSH 或容器需要传输随附修补，并在目标主机运行校验脚本。
...
cli-agent-grok-image-format-unverified = Grok 托管任务目前仅支持已验证 Grok Build 1.0.41 与 grok-4.7 的 PNG 图片。草稿已保留。
cli-agent-claude-image-skill-unverified = 此版本尚未验证图片与技能混用，请分轮提交。
cli-agent-input-text-too-large = 文本输入大小须不超过 1 MiB。
cli-agent-runtime-data-too-large = CLI 数据超过大小上限。
cli-agent-claude-image-replay-invalid = CLI 返回的图片输入与当前轮不匹配。
...
cli-agent-task-skills-require-managed = 技能传递需要托管 Codex、Claude 或 Grok 任务。
...
cli-agent-claude-file-policy-unverified = 无法验证 Claude 文件任务策略。请先检查 CLI 权限设置，再新建或继续任务。
cli-task-manager-show-permission-details = 显示权限详情
cli-task-manager-hide-permission-details = 收起权限详情
...
cli-task-manager-grok-queued = Grok 的追加指令等待后续回合，只有 Grok 接收后才确认收到。启用本地任务工具时，取消本轮会关闭连接，等待中的指令不会发送，需明确继续任务。未启用时，等待中的指令可能在取消后开始。

cli-task-manager-permission-grok-read = Grok：固定只读工具与逐次审批
cli-task-manager-permission-grok-files = Grok：固定文件工具与逐次审批
cli-task-manager-permission-grok-p0 = Grok：已验证的 P0 原生读取
cli-task-manager-permission-grok-read-help = 使用应用专属的 Grok 配置，固定只读工具，每次调用均需审批。钩子、技能和终端命令均已禁用。这不是文件系统沙箱，系统管理员策略仍然适用。
...
settings-cli-updates-channel-stable = Stable（稳定版）
settings-cli-updates-channel-alpha = Alpha（预览版）
settings-cli-updates-channel-codex = Latest 为正式版，Alpha 包含预览功能。选择跟随安装时保留当前渠道。
settings-cli-updates-channel-claude = Latest 为官方默认渠道，Stable 延迟接收更新。更新时会同步所选渠道到 Claude 设置；跟随安装时保留其渠道。
settings-cli-updates-channel-grok = Stable 为正式版，Alpha 为预览版。更新时会同步所选渠道到 Grok 设置；跟随安装时保留其渠道。
settings-cli-updates-effective-channel = 已识别的目标渠道：{ $channel }
settings-cli-updates-not-checked = 尚未检查更新。
settings-cli-updates-version-unknown = 未知
settings-cli-updates-versions = 已安装：{ $installed } · 目标版本：{ $latest } · 安装来源：{ $source }
settings-cli-updates-check = 检查更新
settings-cli-updates-update = 空闲时更新
settings-cli-updates-install-guide = 安装指南
settings-cli-updates-source-native = 官方安装器
settings-cli-updates-source-unknown = 未识别
settings-cli-updates-checking = 正在检查所选发布渠道…
settings-cli-updates-current = 当前安装已符合所选渠道。
...
settings-cli-updates-probe-failed = 无法确认 CLI 版本。请检查安装和更新状态后重试。
settings-cli-updates-permission = 当前权限无法更新此安装。请使用原安装工具。
settings-cli-updates-command-failed = 安装器报告失败。请检查安装后重试。
settings-cli-updates-timeout = 更新超时。请检查安装后重试。
settings-cli-updates-version-mismatch = 已安装版本与所选目标不符。请重新检查后重试。
settings-cli-updates-channel-mismatch = 此安装来源无法自动切换到所选渠道。请使用官方安装器切换渠道。
settings-cli-updates-recovery = 上次更新需要恢复。请检查更新以确认安装状态后重试。
settings-cli-updates-persistence = 无法保存更新记录。请检查本地存储后重试。
