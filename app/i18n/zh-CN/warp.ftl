...
cli-agent-grok-managed-unverified = 已验证的 Grok 托管接口不支持这项操作或权限策略。根任务请继承 CLI 设置；SDK 工具、子任务、图片和技能仍未验证。
...
cli-task-manager-grok-unavailable = Grok Build 1.0.30 支持继承 CLI 设置的根文本任务、审批、取消与历史继续。追加指令等待后续回合；SDK 工具、子任务、图片和技能暂不可用。
...
cli-agent-grok-input-skills-unverified = Grok Build 托管技能输入尚未验证，请在草稿中保留所选技能。
cli-task-manager-grok-queued = Grok 运行时，追加指令等待后续回合，不会修改当前执行。只有 Grok 接收该指令后才确认收到。取消当前回合后，等待的指令可能随后开始。
