...
cli-task-manager-queue-next-turn = 排队下一轮
cli-task-manager-claude-queued = Claude 有一条指令尚未开始。确认收到不表示已经执行。取消当前回合只中断正在运行的一轮，排队指令仍可能执行。
cli-agent-claude-queue-full = Claude 已有一条指令等待开始。请保留草稿，等该回合开始后再发送。
