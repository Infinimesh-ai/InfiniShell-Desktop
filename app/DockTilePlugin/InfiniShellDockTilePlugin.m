#include "InfiniShellDockTilePlugin.h"

@implementation InfiniShellDockTilePlugIn {
...
- (instancetype)init {
    ...
    if (self) {
        @try {
            ...
            [self logMessage:@"InfiniShellDockTilePlugin initialized"];
...
- (void)setDockTile:(NSDockTile *)dockTile {
    @try {
        ...
        if (dockTile) {
            ...
            NSBundle *pluginBundle = [NSBundle bundleForClass:[InfiniShellDockTilePlugIn class]];    
            NSString *path = [[pluginBundle bundlePath] stringByAppendingPathComponent:@"Contents/Info.plist"];
            NSDictionary *dict = [NSDictionary dictionaryWithContentsOfFile:path];    
            NSString *bundleId = dict[@"MainAppBundleIdentifier"];
...
- (void)dealloc {
    @try {
        [self logMessage:@"InfiniShellDockTilePlugin deallocating"];
