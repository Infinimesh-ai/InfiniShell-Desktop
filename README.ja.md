...
InfiniShell はオープンでローカルファーストなターミナルで、AI と Agent をファーストクラスでサポートします。任意の AI プロバイダーを接続し、任意の CLI Agent を取り込み、ターミナル内で SSH ホストを管理 —— API キー・履歴・Agent の状態はデフォルトで自分のマシンに留まります。

## 公式 Warp に対して InfiniShell が追加する機能
...
## Zap / OpenWarp / Warp からの移行

直前の名称である **Zap** からのアップグレードでは何もする必要はありません ——
InfiniShell が初回起動時に旧 `.zap` ディレクトリを自動的に移行します。

**OpenWarp** の時代から使っていた方、または上流 **Warp** から乗り換える方は、
...
- [Warp](https://github.com/warpdotdev/warp) —— InfiniShell がベースとしている上流のターミナル。
