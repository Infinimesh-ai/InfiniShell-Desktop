...
use crate::proto::{
    ...
    CodebaseIndexStatusesSnapshot, ErrorCode, FileSystemEntryKind, GetDiffStateResponse,
    ...
    RunCommandResponse, RunCommandSuccess, ServerMessage, WriteFile, WriteFileChunkResponse,
    WriteFileChunkSuccess, client_message, host_scoped_request, notification,
...
async fn resolve_path_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let host_scoped_request::Message::ResolvePath(req) = unwrap_host_scoped(msg) else {
            panic!("Expected ResolvePath");
        };
...
async fn read_file_chunk_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let host_scoped_request::Message::ReadFileChunk(req) = unwrap_host_scoped(msg) else {
            panic!("Expected ReadFileChunk");
        };
...
async fn write_file_chunk_round_trip() {
    let (client, _disconnect_rx, _executor) = setup_mock_client(|msg| {
        let host_scoped_request::Message::WriteFileChunk(req) = unwrap_host_scoped(msg) else {
            panic!("Expected WriteFileChunk");
        };
        assert_eq!(req.path, "/tmp/blob.bin");
