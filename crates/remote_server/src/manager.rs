...
impl RemoteServerErrorKind {
    ...
    pub fn from_client_error(error: &crate::client::ClientError) -> Self {
        ...
        match error {
            ...
            ClientError::ServerError { .. } | ClientError::FileOperationFailed(_) => {
                Self::ServerError
            }
...
impl From<crate::client::ClientError> for HostRequestError {
    fn from(err: crate::client::ClientError) -> Self {
        ...
        match err {
            ...
            // SSH 文件浏览器(local_fs)的域级失败:服务端已应答,只是操作本身失败,
            // 消息按原样透传给调用方展示。
            ClientError::FileOperationFailed(message) => Self::OperationFailed(message),
...
impl RemoteServerManager {
    ...
    async fn run_connect_and_handshake(
        ...
        let resp = client
            .initialize(
                auth_token.as_deref(),
                InitializeParams {
                    // Zap 没有账号体系,也已剥离崩溃上报(sentry),握手不携带任何
                    // 用户身份;远端据此保持匿名并禁用崩溃上报。
                    user_id: String::new(),
    ...
    pub(crate) fn mark_session_disconnected(
        ...
        } = prev
        {
            ...
            //
            // Zap 在上游判定之上进一步收紧:只要子进程已经退出
            // (`exit_status.is_some()`),就说明对端 daemon 已经真的不在了
            // —— 例如用户在远端 shell 内 `exit`,ssh ControlMaster 把所有
            // slave channel 一起收掉,`remote-server-proxy` 也跟着退出。
            ...
            if child_already_exited || !transport.is_reconnectable(exit_status.as_ref()) {
                log::warn!(
                    "Disconnect is not reconnectable, skipping reconnect: session={session_id:?} exit_status={exit_status:?}"
                );
