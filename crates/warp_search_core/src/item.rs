...
pub trait SearchItem: Send + Sync {
    ...
    ///
    /// Items that return the same key are considered duplicates by
    /// [`crate::mixer::DedupeStrategy::HighestScore`]. Returning `None` (the default) opts the
    /// item out of deduplication.
    // Zap:WelcomePalette 上游随欢迎页(#12614)一并删除了去重能力,我方保留欢迎页,故恢复。
