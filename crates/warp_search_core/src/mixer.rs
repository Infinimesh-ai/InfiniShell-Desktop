...
/// How the mixer should handle results that share the same [`QueryResult::dedup_key`].
// Zap:WelcomePalette 上游随欢迎页(#12614)一并删除了去重能力,我方保留欢迎页(项目列表与
// "建议的项目"两个数据源会产出同一路径),故恢复。
...
impl<T: Action + Clone> SearchMixer<T> {
    ...
    /// Sets the strategy used to deduplicate results that share a [`QueryResult::dedup_key`].
