...
lazy_static! {
    ...
    ///
    /// `claude`, `codex`, `gemini`, `infinishell-tui`, and legacy `warp` are not
    /// actually _really_ one-off shell command
    /// keywords, but false-positive NL classifications for these inputs (where the user was trying
    /// to use claude code, codex CLI, gemini CLI, or InfiniShell TUI) suck, because the user
    /// often thinks we're intentionally trying to push them away from those CLIs into Agent Mode,
    /// so we mitigate the risk by always treating them as shell commands.
    static ref ONE_OFF_SHELL_COMMAND_KEYWORDS: HashSet<&'static str> = HashSet::from(["#", "echo", "man", "sudo", "claude", "codex", "gemini", "agy", "omp", "warp", "infinishell-tui"]);
