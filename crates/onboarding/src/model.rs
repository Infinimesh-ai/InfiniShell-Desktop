...
/// Auth / billing state of the user, as far as onboarding cares about it.
///
/// Zap has no accounts or billing, so this stays `LoggedOut` in practice; the
/// type is kept because the upstream account-first slides still model it.
...
impl OnboardingStateModel {
    ...
    pub(crate) fn set_auth_state(
        ...
        // Zap 没有账号体系,没有 AuthStateChanged 事件可发,只刷新视图。
        ctx.notify();
    }
    ...
    pub(crate) fn request_credit_purchase(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Zap 没有计费后端,也就没有 CreditPurchaseRequested 事件的监听方;
        // `credit_pack_options` 始终为空,这里必然提前返回,后面的状态迁移
        // 实际不可达。
        let Some(_pack) = self.selected_credit_pack() else {
            return;
        };
    ...
    /// 上游在这里向 app 发 `UpgradeRequested` 去拉付费升级流程。Zap 没有账号和
    /// 计费,所以保留调用点但不做任何事。
    pub(crate) fn request_upgrade(&mut self, _ctx: &mut ModelContext<Self>) {}
