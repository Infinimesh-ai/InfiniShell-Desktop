    ...
    fn convert_window_event(
        ...
        match evt {
            ...
            WindowEvent::KeyboardInput {
                ...
                // 转换失败时走 Zap 的文本回退路径:除覆盖上游针对 Unidentified 键
                // (如 Windows 上 Unikey/EVKey 经 SendInput 注入的合成字符)的场景外,
                // 还处理移动端/WASM 软键盘等只带文本的按键事件。
    ...
    fn handle_converted_warpui_event(
            ...
            } = event
        {
            ...
            if position.y() < titlebar_height && !cfg!(target_family = "wasm") {
                ...
                } else if window_state.last_touch_purpose.is_none() && !winit_window.is_maximized()
                {
