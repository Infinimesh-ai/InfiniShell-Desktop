...
impl LocalRepoMetadataModel {
    ...
    fn add_repository_internal(
        ...
        // any). Zap:跳过 home 目录及其祖先,避免递归监听无关的用户数据;
        // 这些路径仍然可以出现在文件树里。
        #[cfg(feature = "local_fs")]
        {
    ...
    pub fn remove_repository(
        ...
        if self.remove_repository_state(repo_path).is_some() {
            ...
            // Zap:home 目录及其祖先从未注册过(见 add_repository_internal 的
            // `is_unsafe_watch_root` 门控),这里同样跳过。
            #[cfg(feature = "local_fs")]
            {
                ...
                if let Some(ref watcher) = self.watcher {
                    ...
                    let mut paths_to_unregister: Vec<PathBuf> = repo_path
                        .to_local_path()
                        .filter(|local_path| !is_unsafe_watch_root(local_path))
                        .into_iter()
                        .collect();
    ...
    pub fn load_directory_with_completion(
        ...
        // 构建目录树时会随着遍历修改 gitignore 栈，因此这里仍需要独立的 Vec。
