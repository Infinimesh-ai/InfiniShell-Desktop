...
pub fn default_log_bundle_filename() -> String {
    // LogState 初始化后使用其解析出的日志文件名(区分 GUI/TUI/CLI 前端),
    // 否则回退到 channel 级默认名。
    let logfile_name = LOG_STATE
        .get()
        .map(|state| state.logfile_name.clone())
        .unwrap_or_else(|| ChannelState::logfile_name().into_owned());
